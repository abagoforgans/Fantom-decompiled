...
function sub_3c5ebfaf(?) payable {
    return sub_3c5ebfaf
}

function addressToken() payable {
...
function buy() payable {
    ...
    if mem[176 len 20] >= address(addressToken):
        mem[ceil32(return_data.size) + 233 len 64] = getPair(address arg1, address arg2), Mask(224, 0, stor3), uint32(stor3), address(mem[164]) << 64
        mem[ceil32(return_data.size) + 325 len 4] = 0
        call 0x152ee697f2e276fa89e96742e9bb9ab1f2e61be3 with:
           funct uint32(stor3)
             gas gas_remaining wei
            args address(mem[164]) << 320, mem[ceil32(return_data.size) + 297 len 4]
        if not return_data.size:
        ...
        if address(addressToken) == mem[176 len 20]:
