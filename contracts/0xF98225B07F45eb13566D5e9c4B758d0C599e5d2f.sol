        ...
        if sub_25c7ce6b >= 10000:
            if wantLockedTotal <= 0:
                ...
                if not uint8(stor2.field_16):
                    ...
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                stor26[address(arg1)] = block.timestamp
                stor1 = 1
                return arg2
            if sharesTotal <= 0:
                if sharesTotal + arg2 < sharesTotal:
    ...
    if not arg2:
        ...
        if 0 / wantLockedTotal <= sharesTotal:
            ...
            if sub_9ccccc15 <= 0:
                ...
                if stor26[address(arg1)] + (72 * 24 * 3600) < stor26[address(arg1)]:
                    revert with 0, 'SafeMath: addition overflow'
                if stor26[address(arg1)] + (72 * 24 * 3600) <= block.timestamp:
                    if withdrawFeeFactor >= 10000:
                        if not uint8(stor2.field_16):
                            require ext_code.size(wantAddress)
                            staticcall wantAddress.0x70a08231 with:
