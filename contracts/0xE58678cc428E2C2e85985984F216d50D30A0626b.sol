...
const pairCodeHash = sha3(code.data[1801 len 8836])

const INIT_CODE_PAIR_HASH = sha3(code.data[1801 len 8832], mem[17856 len 4])
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'Syfin: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'Syfin: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'Syfin: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[1801 len 8836]
