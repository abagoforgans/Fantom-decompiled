...
uint256 stor13;
mapping of uint8 sub_eb34080d;
array of uint256 sub_69c2fd1f;
...
function sub_69c2fd1f(?) payable {
    return sub_69c2fd1f[stor14[arg1]][0 len sub_69c2fd1f[stor14[arg1]].length]
}
...
function sub_75f31486(?) payable {
    return sub_69c2fd1f[arg1 << 248][0 len sub_69c2fd1f[arg1 << 248].length]
}
...
function sub_850caa24(?) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    sub_69c2fd1f[cd[4] << 248] = (2 * ('cd', 36).length) + 1
    s = 0
    idx = cd[36] + 36
    while cd[36] + ('cd', 36).length + 36 > idx:
        sub_69c2fd1f[cd[4] << 248][s] = cd[idx]
        s = s + 1
        idx = idx + 32
        continue 
    idx = Mask(251, 0, ('cd', 36).length + 31) >> 5
    while sub_69c2fd1f[cd[4] << 248].length + 31 / 32 > idx:
        sub_69c2fd1f[cd[4] << 248][idx] = 0
        idx = idx + 1
        continue 
}
