                ...
                if not userInfo[address(msg.sender)].field_0:
                    ...
                    if not raisingAmount:
                        ...
                        if userInfo[address(msg.sender)].field_0 - (raisingAmount * 10^12 * userInfo[address(msg.sender)].field_0 / totalAmount / 10^6 / 10^6) > 0:
                            ...
                            if not ext_code.hash(lpTokenAddress):
                                revert with 0, 'Address: call to non-contract'
                            mem[ceil32(return_data.size) + 745 len 64] = 0, msg.sender, Mask(224, 32, userInfo[address(msg.sender)].field_0 - (raisingAmount * 10^12 * userInfo[address(msg.sender)].field_0 / totalAmount / 10^6 / 10^6)) >> 32
                            call lpTokenAddress with:
                               funct uint32(msg.sender)
                                 gas gas_remaining wei
                                args userInfo[address(msg.sender)].field_0 - (raisingAmount * 10^12 * userInfo[address(msg.sender)].field_0 / totalAmount / 10^6 / 10^6), mem[ceil32(return_data.size) + 681 len 28], mem[ceil32(return_data.size) + 809 len 4]
                            if not return_data.size:
                                require not ext_call.success
                                revert with 'SafeMath: division by zero'
                            mem[ceil32(return_data.size) + 777 len return_data.size] = ext_call.return_data[0 len return_data.size]
