...
function removeProxyRegistryAt(uint256 arg1) payable {
    ...
    while idx < allProxyRegistries.length:
        if idx != arg1:
            require idx < allProxyRegistries.length
            stor0.length++
            mem[0] = 0
            address(stor0[stor0.length]) = address(allProxyRegistries[idx])
        idx = idx + 1
        continue 
    allProxyRegistries.length = stor0.length
    if not stor0.length:
        idx = 0
        ...
        while stor0.length > idx:
            uint256(allProxyRegistries[s]) = uint256(stor0[idx])
            s = s + 1
            idx = idx + 1
            continue 
        idx = stor0.length
