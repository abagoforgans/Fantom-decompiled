...
function getLockedBalance(address arg1) payable {
    ...
    while idx < stor15.length:
        ...
        staticcall stor15[idx].field_0.investments(address rg1) with:
        ...
        mem[mem[64] + 4] = arg1
        require ext_code.size(stor15[idx].field_0)
        staticcall stor15[idx].field_0.claimed(address rg1) with:
                gas gas_remaining wei
               args address(arg1)
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
        ...
        if mem[_54] >= 4:
            ...
            continue 
        require ext_code.size(stor15[idx].field_0)
        staticcall stor15[idx].field_0.tokenPriceInWei() with:
                gas gas_remaining wei
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
