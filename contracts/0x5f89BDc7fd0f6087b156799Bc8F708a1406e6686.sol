...
function toggleLock() payable {
    ...
    Mask(248, 0, stor4.field_8) = Mask(248, 0, not bool(canLock))
}

function sub_195a616a(?) payable {
    if owner != msg.sender:
        revert with 0, 'Caller is not owner'
    uint256(stor4.field_0) = not bool(canUnlock) or Mask(248, 8, uint256(stor4.field_0))
}
...
function migrateGton(address arg1, uint256 arg2) payable {
    ...
    call sub_956974acAddress.0xa9059cbb with:
...
function lockTokens(uint256 arg1) payable {
    ...
    require ext_code.size(sub_956974acAddress)
    call sub_956974acAddress.0x23b872dd with:
         gas gas_remaining wei
    ...
    call balanceContractAddress.addValue(address rg1, uint256 rg2) with:
         gas gas_remaining wei
...
function sub_7fde4424(?) payable {
    ...
    require ext_code.size(balanceContractAddress)
    call balanceContractAddress.subtractValue(address rg1, uint256 rg2) with:
         gas gas_remaining wei
        args msg.sender, arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
