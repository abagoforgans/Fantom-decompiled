contract main {




// =====================  Runtime code  =====================


#
#  - sub_cf29a21c(?)
#
address owner;
address sub_ba32e624Address;
address sub_b6a21b61Address;
address sub_a3bf202bAddress;
address sub_242c3c53Address;
address sub_077d66f3Address;
address sub_29dc484aAddress;
address sub_0eebd6a8Address;
address sub_1b85d6deAddress;
address sub_91996877Address;
address sub_7015a9c8Address;
address sub_c472e2c1Address;
address sub_23ba76faAddress;
array of address sub_a5218fd8;
address sub_c052d0abAddress;

function sub_077d66f3(?) {
    return sub_077d66f3Address
}

function sub_0eebd6a8(?) {
    return sub_0eebd6a8Address
}

function sub_1b85d6de(?) {
    return sub_1b85d6deAddress
}

function sub_23ba76fa(?) {
    return sub_23ba76faAddress
}

function sub_242c3c53(?) {
    return sub_242c3c53Address
}

function sub_29dc484a(?) {
    return sub_29dc484aAddress
}

function sub_7015a9c8(?) {
    return sub_7015a9c8Address
}

function getOwner() {
    return owner
}

function sub_91996877(?) {
    return sub_91996877Address
}

function sub_a3bf202b(?) {
    return sub_a3bf202bAddress
}

function sub_a5218fd8(?) {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    require arg1 < sub_a5218fd8.length
    return sub_a5218fd8[arg1]
}

function sub_b6a21b61(?) {
    return sub_b6a21b61Address
}

function sub_ba32e624(?) {
    return sub_ba32e624Address
}

function sub_c052d0ab(?) {
    return sub_c052d0abAddress
}

function sub_c472e2c1(?) {
    return sub_c472e2c1Address
}

function _fallback() payable {
    revert
}

function changeOwner(address arg1) {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    if owner != msg.sender:
        revert with 0, 'Caller is not owner'
    emit OwnerSet(owner, arg1);
    owner = arg1
}

function _approve(address arg1, address arg2, uint256 arg3) {
    require calldata.size - 4 >= 96
    require arg1 == arg1
    require arg2 == arg2
    require arg3 == arg3
    if owner != msg.sender:
        revert with 0, 'Caller is not owner'
    require ext_code.size(arg1)
    call arg1.approve(address arg1, uint256 arg2) with:
         gas gas_remaining wei
        args address(arg2), arg3
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == bool(ext_call.return_data[0])
}

function _allowance(address arg1, address arg2) {
    require calldata.size - 4 >= 64
    require arg1 == arg1
    require arg2 == arg2
    if owner != msg.sender:
        revert with 0, 'Caller is not owner'
    require ext_code.size(arg1)
    staticcall arg1.allowance(address arg1, address arg2) with:
            gas gas_remaining wei
           args address(this.address), arg2
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[0]
    return ext_call.return_data[0]
}

function sub_b22be9c3(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == address(arg1)
    if owner != msg.sender:
        revert with 0, 'Caller is not owner'
    require ext_code.size(this.address)
    staticcall this.address.0x893d20e8 with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[12 len 20]
    if address(arg1) == sub_ba32e624Address:
        call ext_call.return_data[12 len 20] with:
           value eth.balance(this.address) wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        else:
            return 0
    require ext_code.size(address(arg1))
    staticcall address(arg1).balanceOf(address arg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[0]
    require ext_code.size(address(arg1))
    call address(arg1).transfer(address arg1, uint256 arg2) with:
         gas gas_remaining wei
        args address(ext_call.return_data[0]), ext_call.return_data[0]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == bool(ext_call.return_data[0])
    return ext_call.return_data[0]
}

function sub_97aedadb(?) payable {
    require calldata.size - 4 >= 32
    require cd[4] <= test266151307()
    require calldata.size + -cd[4] - 4 >= 224
    require ('cd', 4).length == ('cd', 4).length
    mem[96] = ('cd', 4).length
    require ('cd', 4)[0] == ('cd', 4)[0]
    mem[128] = ('cd', 4)[0]
    require ('cd', 4)[1] <= test266151307()
    require cd[4] + ('cd', 4)[1] + 35 < calldata.size
    if cd[(cd[4] + ('cd', 4)[1] + 4)] > test266151307():
        revert with 'NH{q', 65
    if floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + 321 > test266151307() or floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + 321 < 320:
        revert with 'NH{q', 65
    mem[320] = cd[(cd[4] + ('cd', 4)[1] + 4)]
    require cd[4] + ('cd', 4)[1] + (32 * cd[(cd[4] + ('cd', 4)[1] + 4)]) + 36 <= calldata.size
    idx = 0
    s = cd[4] + ('cd', 4)[1] + 36
    t = 352
    while idx < cd[(cd[4] + ('cd', 4)[1] + 4)]:
        require cd[s] == cd[s]
        mem[t] = cd[s]
        idx = idx + 1
        s = s + 32
        t = t + 32
        continue 
    mem[160] = 320
    require ('cd', 4)[2] <= test266151307()
    require cd[4] + ('cd', 4)[2] + 35 < calldata.size
    if cd[(cd[4] + ('cd', 4)[2] + 4)] > test266151307():
        revert with 'NH{q', 65
    if floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + floor32(cd[(cd[4] + ('cd', 4)[2] + 4)]) + 322 > test266151307() or floor32(cd[(cd[4] + ('cd', 4)[2] + 4)]) + 322 < 321:
        revert with 'NH{q', 65
    mem[64] = floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + floor32(cd[(cd[4] + ('cd', 4)[2] + 4)]) + 322
    mem[floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + 321] = cd[(cd[4] + ('cd', 4)[2] + 4)]
    require cd[4] + ('cd', 4)[2] + (32 * cd[(cd[4] + ('cd', 4)[2] + 4)]) + 36 <= calldata.size
    idx = 0
    s = cd[4] + ('cd', 4)[2] + 36
    t = floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + 353
    while idx < cd[(cd[4] + ('cd', 4)[2] + 4)]:
        require cd[s] <= test266151307()
        require cd[4] + ('cd', 4)[2] + cd[s] + 67 < calldata.size
        if cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)] > test266151307():
            revert with 'NH{q', 65
        _683 = mem[64]
        if mem[64] + floor32(cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]) + 1 > test266151307() or mem[64] + floor32(cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]) + 1 < mem[64]:
            revert with 'NH{q', 65
        mem[64] = mem[64] + floor32(cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]) + 1
        mem[_683] = cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]
        require cd[4] + ('cd', 4)[2] + cd[s] + (32 * cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]) + 68 <= calldata.size
        u = 0
        v = cd[4] + ('cd', 4)[2] + cd[s] + 68
        w = _683 + 32
        while u < cd[(cd[4] + ('cd', 4)[2] + cd[s] + 36)]:
            require cd[v] == uint16(cd[v])
            mem[w] = cd[v]
            u = u + 1
            v = v + 32
            w = w + 32
            continue 
        mem[t] = _683
        idx = idx + 1
        s = s + 32
        t = t + 32
        continue 
    mem[192] = floor32(cd[(cd[4] + ('cd', 4)[1] + 4)]) + 321
    require ('cd', 4)[3] <= test266151307()
    require cd[4] + ('cd', 4)[3] + 35 < calldata.size
    if cd[(cd[4] + ('cd', 4)[3] + 4)] > test266151307():
        revert with 'NH{q', 65
    _684 = mem[64]
    if mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + 4)]) + 1 > test266151307() or mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + 4)]) + 1 < mem[64]:
        revert with 'NH{q', 65
    mem[64] = mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + 4)]) + 1
    mem[_684] = cd[(cd[4] + ('cd', 4)[3] + 4)]
    require cd[4] + ('cd', 4)[3] + (32 * cd[(cd[4] + ('cd', 4)[3] + 4)]) + 36 <= calldata.size
    idx = 0
    s = cd[4] + ('cd', 4)[3] + 36
    t = _684 + 32
    while idx < cd[(cd[4] + ('cd', 4)[3] + 4)]:
        require cd[s] <= test266151307()
        require cd[4] + ('cd', 4)[3] + cd[s] + 67 < calldata.size
        if cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)] > test266151307():
            revert with 'NH{q', 65
        _1206 = mem[64]
        if mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]) + 1 > test266151307() or mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]) + 1 < mem[64]:
            revert with 'NH{q', 65
        mem[64] = mem[64] + floor32(cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]) + 1
        mem[_1206] = cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]
        require cd[4] + ('cd', 4)[3] + cd[s] + (32 * cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]) + 68 <= calldata.size
        u = 0
        v = cd[4] + ('cd', 4)[3] + cd[s] + 68
        w = _1206 + 32
        while u < cd[(cd[4] + ('cd', 4)[3] + cd[s] + 36)]:
            require cd[v] == address(cd[v])
            mem[w] = cd[v]
            u = u + 1
            v = v + 32
            w = w + 32
            continue 
        mem[t] = _1206
        idx = idx + 1
        s = s + 32
        t = t + 32
        continue 
    mem[224] = _684
    require ('cd', 4)[4] <= test266151307()
    require cd[4] + ('cd', 4)[4] + 35 < calldata.size
    if cd[(cd[4] + ('cd', 4)[4] + 4)] > test266151307():
        revert with 'NH{q', 65
    _1207 = mem[64]
    if mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + 4)]) + 1 > test266151307() or mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + 4)]) + 1 < mem[64]:
        revert with 'NH{q', 65
    mem[64] = mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + 4)]) + 1
    mem[_1207] = cd[(cd[4] + ('cd', 4)[4] + 4)]
    require cd[4] + ('cd', 4)[4] + (32 * cd[(cd[4] + ('cd', 4)[4] + 4)]) + 36 <= calldata.size
    idx = 0
    s = cd[4] + ('cd', 4)[4] + 36
    t = _1207 + 32
    while idx < cd[(cd[4] + ('cd', 4)[4] + 4)]:
        require cd[s] <= test266151307()
        require cd[4] + ('cd', 4)[4] + cd[s] + 67 < calldata.size
        if cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)] > test266151307():
            revert with 'NH{q', 65
        _1735 = mem[64]
        if mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]) + 1 > test266151307() or mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]) + 1 < mem[64]:
            revert with 'NH{q', 65
        mem[64] = mem[64] + floor32(cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]) + 1
        mem[_1735] = cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]
        require cd[4] + ('cd', 4)[4] + cd[s] + (32 * cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]) + 68 <= calldata.size
        u = 0
        v = cd[4] + ('cd', 4)[4] + cd[s] + 68
        w = _1735 + 32
        while u < cd[(cd[4] + ('cd', 4)[4] + cd[s] + 36)]:
            require cd[v] == address(cd[v])
            mem[w] = cd[v]
            u = u + 1
            v = v + 32
            w = w + 32
            continue 
        mem[t] = _1735
        idx = idx + 1
        s = s + 32
        t = t + 32
        continue 
    mem[256] = _1207
    require ('cd', 4)[5] == bool(('cd', 4)[5])
    mem[288] = ('cd', 4)[5]
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if 0 >= mem[mem[mem[224] + 32]]:
        revert with 'NH{q', 50
    _1740 = mem[mem[mem[224] + 32] + 32]
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if mem[mem[mem[224] + 32]] < 1:
        revert with 'NH{q', 17
    if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
        revert with 'NH{q', 50
    _1749 = mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 32]
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if 0 >= mem[mem[mem[224] + 32]]:
        revert with 'NH{q', 50
    if mem[mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
        _1756 = mem[128]
        mem[mem[64] + 4] = msg.sender
        mem[mem[64] + 36] = this.address
        mem[mem[64] + 68] = _1756
        require ext_code.size(address(_1740))
        call address(_1740).transferFrom(address arg1, address arg2, uint256 arg3) with:
             gas gas_remaining wei
            args msg.sender, address(this.address), _1756
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _1768 = mem[64]
        mem[64] = mem[64] + ceil32(return_data.size)
        require return_data.size >= 32
        require mem[_1768] == bool(mem[_1768])
        if var217003 < 1:
            revert with 'NH{q', 17
        if uint16(var221002) <= var221001:
            if uint16(var223001) >= mem[mem[160]]:
                revert with 'NH{q', 50
            if uint16(var225003) >= mem[mem[224]]:
                revert with 'NH{q', 50
            _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
            s = mem[224]
            t = var304001
            u = var304003
            while _7837 >= 2:
                if uint16(t) > _7837 - 2:
                    if var225008 > -u - 1:
                        revert with 'NH{q', 17
                    if uint16(var225003) == 65535:
                        revert with 'NH{q', 17
                    if mem[mem[160]] < 1:
                        revert with 'NH{q', 17
                    if uint16(uint16(var225003) + 1) <= mem[mem[160]] - 1:
                        if uint16(uint16(var225003) + 1) >= mem[mem[160]]:
                            revert with 'NH{q', 50
                        if uint16(uint16(var225003) + 1) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
                        s = mem[224]
                        t = 0
                        u = mem[(32 * uint16(uint16(var225003) + 1)) + mem[160] + 32]
                        continue 
                    require var225008 + u >= mem[96]
                    mem[mem[64] + 32] = 9
                    mem[mem[64] + 64] = 'MAN INJAM'
                    emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
                    mem[mem[64]] = var225008 + u
                    emit 0x2f8e8d68: (var225008 + u)
                    if 0 >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if 0 >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if mem[mem[mem[224] + 32]] < 1:
                        revert with 'NH{q', 17
                    if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
                        revert with 'NH{q', 50
                    if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
                        mem[mem[64] + 4] = msg.sender
                        mem[mem[64] + 36] = var225008 + u
                        require ext_code.size(address(_1749))
                        call address(_1749).transfer(address arg1, uint256 arg2) with:
                             gas gas_remaining wei
                            args msg.sender, var225008 + u
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8223 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8223] == bool(mem[_8223])
                    else:
                        if bool(mem[288]) != 1:
                            mem[mem[64] + 4] = msg.sender
                            mem[mem[64] + 36] = var225008 + u
                            require ext_code.size(address(_1749))
                            call address(_1749).transfer(address arg1, uint256 arg2) with:
                                 gas gas_remaining wei
                                args msg.sender, var225008 + u
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8226 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8226] == bool(mem[_8226])
                        else:
                            mem[mem[64] + 32] = 9
                            mem[mem[64] + 64] = 'MAN UNJAM'
                            emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
                            emit 0x2f8e8d68: (var225008 + u)
                            mem[mem[64] + 4] = var225008 + u
                            require ext_code.size(sub_c052d0abAddress)
                            call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                                 gas gas_remaining wei
                                args (var225008 + u)
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8277 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8277] == mem[_8277]
                            emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
                            emit 0x2f8e8d68: mem[_8277]
                            call msg.sender with:
                               value eth.balance(this.address) wei
                                 gas 2300 * is_zero(value) wei
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                    return (var225008 + u)
                if uint16(var225003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] > 7:
                    if uint16(var225003) >= mem[mem[192]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[192] + 32]]:
                        revert with 'NH{q', 50
                    if uint16(var225003) >= mem[mem[192]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[192] + 32]]:
                        revert with 'NH{q', 50
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] != 8:
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] != 9:
                            if uint16(t) == 65535:
                                revert with 'NH{q', 17
                            if uint16(var225003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
                            s = mem[224]
                            t = uint16(t) + 1
                            u = u
                            continue 
                        if uint16(var225003) >= mem[mem[192]]:
                            revert with 'NH{q', 50
                        if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[192] + 32]]:
                            revert with 'NH{q', 50
                        _7935 = mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 32]
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                            revert with 'NH{q', 50
                        mem[0] = 13
                        idx = 0
                        while idx <= 2:
                            mem[mem[64] + 4] = idx
                            require ext_code.size(sub_a5218fd8[uint16(_7935)])
                            call sub_a5218fd8[uint16(_7935)].coins(uint256 arg1) with:
                                 gas gas_remaining wei
                                args idx
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8361 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8361] == mem[_8361 + 12 len 20]
                            if uint16(var225003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                                revert with 'NH{q', 50
                            mem[mem[64] + 4] = idx
                            require ext_code.size(sub_a5218fd8[uint16(_7935)])
                            call sub_a5218fd8[uint16(_7935)].coins(uint256 arg1) with:
                                 gas gas_remaining wei
                                args idx
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[224] + 32] + 44 len 20] != mem[_8361 + 12 len 20]:
                                _8463 = mem[64]
                                mem[64] = mem[64] + ceil32(return_data.size)
                                require return_data.size >= 32
                                require mem[_8463] == mem[_8463 + 12 len 20]
                            else:
                                _8469 = mem[64]
                                mem[64] = mem[64] + ceil32(return_data.size)
                                require return_data.size >= 32
                                require mem[_8469] == mem[_8469 + 12 len 20]
                            if uint16(var225003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            if uint16(t) > 65534:
                                revert with 'NH{q', 17
                            if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                                revert with 'NH{q', 50
                            if idx == -1:
                                revert with 'NH{q', 17
                            _7837 = mem[mem[(32 * uint16(var225003)) + s + 32]]
                            idx = idx + 1
                            continue 
                        mem[mem[64] + 4] = 0
                        mem[mem[64] + 36] = 0
                        mem[mem[64] + 68] = u
                        mem[mem[64] + 100] = 1
                        require ext_code.size(sub_a5218fd8[uint16(_7935)])
                        call sub_a5218fd8[uint16(_7935)].0xe95dc522 with:
                             gas gas_remaining wei
                            args 0, 0, u, 1
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8424 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8424] == mem[_8424]
                        if uint16(t) == 65535:
                            revert with 'NH{q', 17
                        if uint16(var225003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
                        s = mem[224]
                        t = uint16(t) + 1
                        u = mem[_8424]
                        continue 
                    _7905 = mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 32]
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                        revert with 'NH{q', 50
                    mem[0] = 13
                    idx = 0
                    while idx <= 2:
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7905)])
                        call sub_a5218fd8[uint16(_7905)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8363 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8363] == mem[_8363 + 12 len 20]
                        if uint16(var225003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7905)])
                        call sub_a5218fd8[uint16(_7905)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[224] + 32] + 44 len 20] != mem[_8363 + 12 len 20]:
                            _8464 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8464] == mem[_8464 + 12 len 20]
                        else:
                            _8470 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8470] == mem[_8470 + 12 len 20]
                        if uint16(var225003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) > 65534:
                            revert with 'NH{q', 17
                        if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        if idx == -1:
                            revert with 'NH{q', 17
                        _7837 = mem[mem[(32 * uint16(var225003)) + s + 32]]
                        idx = idx + 1
                        continue 
                    mem[mem[64] + 4] = 0
                    require ext_code.size(sub_a5218fd8[uint16(_7905)])
                    staticcall sub_a5218fd8[uint16(_7905)].balances(uint256 arg1) with:
                            gas gas_remaining wei
                           args 0
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8370 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    _8382 = mem[_8370]
                    require mem[_8370] == mem[_8370]
                    mem[mem[64] + 36] = 0
                    mem[mem[64] + 68] = u
                    mem[mem[64] + 100] = 1
                    require ext_code.size(sub_a5218fd8[uint16(_7905)])
                    call sub_a5218fd8[uint16(_7905)].0xe95dc522 with:
                         gas gas_remaining wei
                        args 0, 0, u, 1
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    mem[mem[64] + 4] = 0
                    require ext_code.size(sub_a5218fd8[uint16(_7905)])
                    staticcall sub_a5218fd8[uint16(_7905)].balances(uint256 arg1) with:
                            gas gas_remaining wei
                           args 0
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8561 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_8561] == mem[_8561]
                    if _8382 < mem[_8561]:
                        revert with 'NH{q', 17
                    if uint16(t) == 65535:
                        revert with 'NH{q', 17
                    if uint16(var225003) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
                    s = mem[224]
                    t = uint16(t) + 1
                    u = _8382 - mem[_8561]
                    continue 
                _7870 = mem[64]
                mem[mem[64] + 32 len 64] = call.data[calldata.size len 64]
                mem[mem[64] + 96] = 2
                mem[64] = mem[64] + 192
                mem[_7870 + 128 len 64] = call.data[calldata.size len 64]
                if uint16(var225003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                    revert with 'NH{q', 50
                if 0 >= mem[_7870 + 96]:
                    revert with 'NH{q', 50
                mem[_7870 + 128] = mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[224] + 32] + 44 len 20]
                if uint16(var225003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if uint16(t) > 65534:
                    revert with 'NH{q', 17
                if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]:
                    revert with 'NH{q', 50
                if 1 >= mem[_7870 + 96]:
                    revert with 'NH{q', 50
                mem[_7870 + 160] = mem[(32 * uint16(uint16(t) + 1)) + mem[(32 * uint16(var225003)) + mem[224] + 32] + 44 len 20]
                if uint16(var225003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var225003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                _8031 = mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 32]
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var225003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                    revert with 'NH{q', 50
                mem[0] = 13
                if 3600 > -block.timestamp - 1:
                    revert with 'NH{q', 17
                mem[_7870 + 192] = 0x38ed173900000000000000000000000000000000000000000000000000000000
                mem[_7870 + 196] = u
                mem[_7870 + 228] = 1
                mem[_7870 + 260] = 160
                mem[_7870 + 356] = mem[_7870 + 96]
                idx = 0
                u = _7870 + 128
                v = _7870 + 388
                while idx < mem[_7870 + 96]:
                    mem[v] = mem[u + 12 len 20]
                    _7837 = mem[mem[(32 * uint16(var225003)) + s + 32]]
                    idx = idx + 1
                    u = u + 32
                    v = v + 32
                    continue 
                mem[_7870 + 292] = this.address
                mem[_7870 + 324] = block.timestamp + 3600
                require ext_code.size(sub_a5218fd8[uint16(_8031)])
                call sub_a5218fd8[uint16(_8031)].mem[mem[64] len 4] with:
                     gas gas_remaining wei
                    args mem[mem[64] + 4 len _7870 + (32 * mem[_7870 + 96]) + -mem[64] + 384]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _8385 = mem[64]
                mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                _8388 = mem[_8385]
                require mem[_8385] <= test266151307()
                require _8385 + mem[_8385] + 31 < _8385 + return_data.size
                _8391 = mem[_8385 + mem[_8385]]
                if mem[_8385 + mem[_8385]] > test266151307():
                    revert with 'NH{q', 65
                if _8385 + ceil32(return_data.size) + floor32(mem[_8385 + mem[_8385]]) + 1 > test266151307() or floor32(mem[_8385 + mem[_8385]]) + 1 < 0:
                    revert with 'NH{q', 65
                mem[64] = _8385 + ceil32(return_data.size) + floor32(mem[_8385 + mem[_8385]]) + 1
                mem[_8385 + ceil32(return_data.size)] = _8391
                require _8388 + (32 * _8391) + 32 <= return_data.size
                idx = 0
                u = _8385 + _8388 + 32
                v = _8385 + ceil32(return_data.size) + 32
                while idx < _8391:
                    require mem[u] == mem[u]
                    mem[v] = mem[u]
                    _7837 = mem[mem[(32 * uint16(var225003)) + s + 32]]
                    idx = idx + 1
                    u = u + 32
                    v = v + 32
                    continue 
                if 1 >= _8391:
                    revert with 'NH{q', 50
                if uint16(t) == 65535:
                    revert with 'NH{q', 17
                if uint16(var225003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                _7837 = mem[mem[(32 * uint16(var225003)) + mem[224] + 32]]
                s = mem[224]
                t = uint16(t) + 1
                u = mem[_8385 + ceil32(return_data.size) + 64]
                continue 
            revert with 'NH{q', 17
        require var221007 >= mem[96]
        mem[mem[64] + 32] = 9
        mem[mem[64] + 64] = 'MAN INJAM'
        emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
        mem[mem[64]] = var221007
        emit 0x2f8e8d68: var221007
        if 0 >= mem[mem[224]]:
            revert with 'NH{q', 50
        if 0 >= mem[mem[224]]:
            revert with 'NH{q', 50
        if mem[mem[mem[224] + 32]] < 1:
            revert with 'NH{q', 17
        if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
            revert with 'NH{q', 50
        if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
            mem[mem[64] + 4] = msg.sender
            mem[mem[64] + 36] = var221007
            require ext_code.size(address(_1749))
            call address(_1749).transfer(address arg1, uint256 arg2) with:
                 gas gas_remaining wei
                args msg.sender, var221007
            mem[mem[64]] = ext_call.return_data[0]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _5976 = mem[64]
            mem[64] = mem[64] + ceil32(return_data.size)
            require return_data.size >= 32
            require mem[_5976] == bool(mem[_5976])
        else:
            if bool(('cd', 4)[5]) != 1:
                mem[mem[64] + 4] = msg.sender
                mem[mem[64] + 36] = var221007
                require ext_code.size(address(_1749))
                call address(_1749).transfer(address arg1, uint256 arg2) with:
                     gas gas_remaining wei
                    args msg.sender, var221007
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _5985 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                require mem[_5985] == bool(mem[_5985])
            else:
                mem[mem[64] + 32] = 9
                mem[mem[64] + 64] = 'MAN UNJAM'
                emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
                emit 0x2f8e8d68: var221007
                mem[mem[64] + 4] = var221007
                require ext_code.size(sub_c052d0abAddress)
                call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                     gas gas_remaining wei
                    args var221007
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _6069 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                require mem[_6069] == mem[_6069]
                emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
                emit 0x2f8e8d68: mem[_6069]
                call msg.sender with:
                   value eth.balance(this.address) wei
                     gas 2300 * is_zero(value) wei
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
        return var221007
    if msg.value == mem[128]:
        require ext_code.size(sub_b6a21b61Address)
        call sub_b6a21b61Address.deposit() with:
           value msg.value wei
             gas gas_remaining wei
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _1762 = mem[64]
        mem[64] = mem[64] + ceil32(return_data.size)
        require return_data.size >= 32
        require mem[_1762] == mem[_1762]
        if var200003 < 1:
            revert with 'NH{q', 17
        if uint16(var204002) <= var204001:
            if uint16(var206001) >= mem[mem[160]]:
                revert with 'NH{q', 50
            if uint16(var208003) >= mem[mem[224]]:
                revert with 'NH{q', 50
            _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
            s = mem[224]
            t = var287001
            u = var287003
            while _7841 >= 2:
                if uint16(t) > _7841 - 2:
                    if var208008 > -u - 1:
                        revert with 'NH{q', 17
                    if uint16(var208003) == 65535:
                        revert with 'NH{q', 17
                    if mem[mem[160]] < 1:
                        revert with 'NH{q', 17
                    if uint16(uint16(var208003) + 1) <= mem[mem[160]] - 1:
                        if uint16(uint16(var208003) + 1) >= mem[mem[160]]:
                            revert with 'NH{q', 50
                        if uint16(uint16(var208003) + 1) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
                        s = mem[224]
                        t = 0
                        u = mem[(32 * uint16(uint16(var208003) + 1)) + mem[160] + 32]
                        continue 
                    require var208008 + u >= mem[96]
                    mem[mem[64] + 32] = 9
                    mem[mem[64] + 64] = 'MAN INJAM'
                    emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
                    mem[mem[64]] = var208008 + u
                    emit 0x2f8e8d68: (var208008 + u)
                    if 0 >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if 0 >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if mem[mem[mem[224] + 32]] < 1:
                        revert with 'NH{q', 17
                    if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
                        revert with 'NH{q', 50
                    if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
                        mem[mem[64] + 4] = msg.sender
                        mem[mem[64] + 36] = var208008 + u
                        require ext_code.size(address(_1749))
                        call address(_1749).transfer(address arg1, uint256 arg2) with:
                             gas gas_remaining wei
                            args msg.sender, var208008 + u
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8225 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8225] == bool(mem[_8225])
                    else:
                        if bool(mem[288]) != 1:
                            mem[mem[64] + 4] = msg.sender
                            mem[mem[64] + 36] = var208008 + u
                            require ext_code.size(address(_1749))
                            call address(_1749).transfer(address arg1, uint256 arg2) with:
                                 gas gas_remaining wei
                                args msg.sender, var208008 + u
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8234 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8234] == bool(mem[_8234])
                        else:
                            mem[mem[64] + 32] = 9
                            mem[mem[64] + 64] = 'MAN UNJAM'
                            emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
                            emit 0x2f8e8d68: (var208008 + u)
                            mem[mem[64] + 4] = var208008 + u
                            require ext_code.size(sub_c052d0abAddress)
                            call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                                 gas gas_remaining wei
                                args (var208008 + u)
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8279 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8279] == mem[_8279]
                            emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
                            emit 0x2f8e8d68: mem[_8279]
                            call msg.sender with:
                               value eth.balance(this.address) wei
                                 gas 2300 * is_zero(value) wei
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                    return (var208008 + u)
                if uint16(var208003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] > 7:
                    if uint16(var208003) >= mem[mem[192]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[192] + 32]]:
                        revert with 'NH{q', 50
                    if uint16(var208003) >= mem[mem[192]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[192] + 32]]:
                        revert with 'NH{q', 50
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] != 8:
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] != 9:
                            if uint16(t) == 65535:
                                revert with 'NH{q', 17
                            if uint16(var208003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
                            s = mem[224]
                            t = uint16(t) + 1
                            u = u
                            continue 
                        if uint16(var208003) >= mem[mem[192]]:
                            revert with 'NH{q', 50
                        if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[192] + 32]]:
                            revert with 'NH{q', 50
                        _7943 = mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 32]
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                            revert with 'NH{q', 50
                        mem[0] = 13
                        idx = 0
                        while idx <= 2:
                            mem[mem[64] + 4] = idx
                            require ext_code.size(sub_a5218fd8[uint16(_7943)])
                            call sub_a5218fd8[uint16(_7943)].coins(uint256 arg1) with:
                                 gas gas_remaining wei
                                args idx
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _8367 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8367] == mem[_8367 + 12 len 20]
                            if uint16(var208003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                                revert with 'NH{q', 50
                            mem[mem[64] + 4] = idx
                            require ext_code.size(sub_a5218fd8[uint16(_7943)])
                            call sub_a5218fd8[uint16(_7943)].coins(uint256 arg1) with:
                                 gas gas_remaining wei
                                args idx
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[224] + 32] + 44 len 20] != mem[_8367 + 12 len 20]:
                                _8467 = mem[64]
                                mem[64] = mem[64] + ceil32(return_data.size)
                                require return_data.size >= 32
                                require mem[_8467] == mem[_8467 + 12 len 20]
                            else:
                                _8473 = mem[64]
                                mem[64] = mem[64] + ceil32(return_data.size)
                                require return_data.size >= 32
                                require mem[_8473] == mem[_8473 + 12 len 20]
                            if uint16(var208003) >= mem[mem[224]]:
                                revert with 'NH{q', 50
                            if uint16(t) > 65534:
                                revert with 'NH{q', 17
                            if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                                revert with 'NH{q', 50
                            if idx == -1:
                                revert with 'NH{q', 17
                            _7841 = mem[mem[(32 * uint16(var208003)) + s + 32]]
                            idx = idx + 1
                            continue 
                        mem[mem[64] + 4] = 0
                        mem[mem[64] + 36] = 0
                        mem[mem[64] + 68] = u
                        mem[mem[64] + 100] = 1
                        require ext_code.size(sub_a5218fd8[uint16(_7943)])
                        call sub_a5218fd8[uint16(_7943)].0xe95dc522 with:
                             gas gas_remaining wei
                            args 0, 0, u, 1
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8430 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8430] == mem[_8430]
                        if uint16(t) == 65535:
                            revert with 'NH{q', 17
                        if uint16(var208003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
                        s = mem[224]
                        t = uint16(t) + 1
                        u = mem[_8430]
                        continue 
                    _7907 = mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 32]
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                        revert with 'NH{q', 50
                    mem[0] = 13
                    idx = 0
                    while idx <= 2:
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7907)])
                        call sub_a5218fd8[uint16(_7907)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8369 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8369] == mem[_8369 + 12 len 20]
                        if uint16(var208003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7907)])
                        call sub_a5218fd8[uint16(_7907)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[224] + 32] + 44 len 20] != mem[_8369 + 12 len 20]:
                            _8468 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8468] == mem[_8468 + 12 len 20]
                        else:
                            _8474 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8474] == mem[_8474 + 12 len 20]
                        if uint16(var208003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) > 65534:
                            revert with 'NH{q', 17
                        if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        if idx == -1:
                            revert with 'NH{q', 17
                        _7841 = mem[mem[(32 * uint16(var208003)) + s + 32]]
                        idx = idx + 1
                        continue 
                    mem[mem[64] + 4] = 0
                    require ext_code.size(sub_a5218fd8[uint16(_7907)])
                    staticcall sub_a5218fd8[uint16(_7907)].balances(uint256 arg1) with:
                            gas gas_remaining wei
                           args 0
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8372 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    _8384 = mem[_8372]
                    require mem[_8372] == mem[_8372]
                    mem[mem[64] + 36] = 0
                    mem[mem[64] + 68] = u
                    mem[mem[64] + 100] = 1
                    require ext_code.size(sub_a5218fd8[uint16(_7907)])
                    call sub_a5218fd8[uint16(_7907)].0xe95dc522 with:
                         gas gas_remaining wei
                        args 0, 0, u, 1
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    mem[mem[64] + 4] = 0
                    require ext_code.size(sub_a5218fd8[uint16(_7907)])
                    staticcall sub_a5218fd8[uint16(_7907)].balances(uint256 arg1) with:
                            gas gas_remaining wei
                           args 0
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8571 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_8571] == mem[_8571]
                    if _8384 < mem[_8571]:
                        revert with 'NH{q', 17
                    if uint16(t) == 65535:
                        revert with 'NH{q', 17
                    if uint16(var208003) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
                    s = mem[224]
                    t = uint16(t) + 1
                    u = _8384 - mem[_8571]
                    continue 
                _7874 = mem[64]
                mem[mem[64] + 32 len 64] = call.data[calldata.size len 64]
                mem[mem[64] + 96] = 2
                mem[64] = mem[64] + 192
                mem[_7874 + 128 len 64] = call.data[calldata.size len 64]
                if uint16(var208003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                    revert with 'NH{q', 50
                if 0 >= mem[_7874 + 96]:
                    revert with 'NH{q', 50
                mem[_7874 + 128] = mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[224] + 32] + 44 len 20]
                if uint16(var208003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if uint16(t) > 65534:
                    revert with 'NH{q', 17
                if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]:
                    revert with 'NH{q', 50
                if 1 >= mem[_7874 + 96]:
                    revert with 'NH{q', 50
                mem[_7874 + 160] = mem[(32 * uint16(uint16(t) + 1)) + mem[(32 * uint16(var208003)) + mem[224] + 32] + 44 len 20]
                if uint16(var208003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var208003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                _8033 = mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 32]
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var208003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                    revert with 'NH{q', 50
                mem[0] = 13
                if 3600 > -block.timestamp - 1:
                    revert with 'NH{q', 17
                mem[_7874 + 192] = 0x38ed173900000000000000000000000000000000000000000000000000000000
                mem[_7874 + 196] = u
                mem[_7874 + 228] = 1
                mem[_7874 + 260] = 160
                mem[_7874 + 356] = mem[_7874 + 96]
                idx = 0
                u = _7874 + 128
                v = _7874 + 388
                while idx < mem[_7874 + 96]:
                    mem[v] = mem[u + 12 len 20]
                    _7841 = mem[mem[(32 * uint16(var208003)) + s + 32]]
                    idx = idx + 1
                    u = u + 32
                    v = v + 32
                    continue 
                mem[_7874 + 292] = this.address
                mem[_7874 + 324] = block.timestamp + 3600
                require ext_code.size(sub_a5218fd8[uint16(_8033)])
                call sub_a5218fd8[uint16(_8033)].mem[mem[64] len 4] with:
                     gas gas_remaining wei
                    args mem[mem[64] + 4 len _7874 + (32 * mem[_7874 + 96]) + -mem[64] + 384]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _8387 = mem[64]
                mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                _8390 = mem[_8387]
                require mem[_8387] <= test266151307()
                require _8387 + mem[_8387] + 31 < _8387 + return_data.size
                _8393 = mem[_8387 + mem[_8387]]
                if mem[_8387 + mem[_8387]] > test266151307():
                    revert with 'NH{q', 65
                if _8387 + ceil32(return_data.size) + floor32(mem[_8387 + mem[_8387]]) + 1 > test266151307() or floor32(mem[_8387 + mem[_8387]]) + 1 < 0:
                    revert with 'NH{q', 65
                mem[64] = _8387 + ceil32(return_data.size) + floor32(mem[_8387 + mem[_8387]]) + 1
                mem[_8387 + ceil32(return_data.size)] = _8393
                require _8390 + (32 * _8393) + 32 <= return_data.size
                idx = 0
                u = _8387 + _8390 + 32
                v = _8387 + ceil32(return_data.size) + 32
                while idx < _8393:
                    require mem[u] == mem[u]
                    mem[v] = mem[u]
                    _7841 = mem[mem[(32 * uint16(var208003)) + s + 32]]
                    idx = idx + 1
                    u = u + 32
                    v = v + 32
                    continue 
                if 1 >= _8393:
                    revert with 'NH{q', 50
                if uint16(t) == 65535:
                    revert with 'NH{q', 17
                if uint16(var208003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                _7841 = mem[mem[(32 * uint16(var208003)) + mem[224] + 32]]
                s = mem[224]
                t = uint16(t) + 1
                u = mem[_8387 + ceil32(return_data.size) + 64]
                continue 
            revert with 'NH{q', 17
        require var204007 >= mem[96]
        mem[mem[64] + 32] = 9
        mem[mem[64] + 64] = 'MAN INJAM'
        emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
        mem[mem[64]] = var204007
        emit 0x2f8e8d68: var204007
        if 0 >= mem[mem[224]]:
            revert with 'NH{q', 50
        if 0 >= mem[mem[224]]:
            revert with 'NH{q', 50
        if mem[mem[mem[224] + 32]] < 1:
            revert with 'NH{q', 17
        if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
            revert with 'NH{q', 50
        if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
            mem[mem[64] + 4] = msg.sender
            mem[mem[64] + 36] = var204007
            require ext_code.size(address(_1749))
            call address(_1749).transfer(address arg1, uint256 arg2) with:
                 gas gas_remaining wei
                args msg.sender, var204007
            mem[mem[64]] = ext_call.return_data[0]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _5982 = mem[64]
            mem[64] = mem[64] + ceil32(return_data.size)
            require return_data.size >= 32
            require mem[_5982] == bool(mem[_5982])
        else:
            if bool(('cd', 4)[5]) != 1:
                mem[mem[64] + 4] = msg.sender
                mem[mem[64] + 36] = var204007
                require ext_code.size(address(_1749))
                call address(_1749).transfer(address arg1, uint256 arg2) with:
                     gas gas_remaining wei
                    args msg.sender, var204007
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _5991 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                require mem[_5991] == bool(mem[_5991])
            else:
                mem[mem[64] + 32] = 9
                mem[mem[64] + 64] = 'MAN UNJAM'
                emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
                emit 0x2f8e8d68: var204007
                mem[mem[64] + 4] = var204007
                require ext_code.size(sub_c052d0abAddress)
                call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                     gas gas_remaining wei
                    args var204007
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _6073 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                require mem[_6073] == mem[_6073]
                emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
                emit 0x2f8e8d68: mem[_6073]
                call msg.sender with:
                   value eth.balance(this.address) wei
                     gas 2300 * is_zero(value) wei
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
        return var204007
    _1758 = mem[128]
    mem[mem[64] + 4] = msg.sender
    mem[mem[64] + 36] = this.address
    mem[mem[64] + 68] = _1758
    require ext_code.size(address(_1740))
    call address(_1740).transferFrom(address arg1, address arg2, uint256 arg3) with:
         gas gas_remaining wei
        args msg.sender, address(this.address), _1758
    mem[mem[64]] = ext_call.return_data[0]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    _1769 = mem[64]
    mem[64] = mem[64] + ceil32(return_data.size)
    require return_data.size >= 32
    require mem[_1769] == bool(mem[_1769])
    if var218003 < 1:
        revert with 'NH{q', 17
    if uint16(var222002) <= var222001:
        if uint16(var224001) >= mem[mem[160]]:
            revert with 'NH{q', 50
        if uint16(var226003) >= mem[mem[224]]:
            revert with 'NH{q', 50
        _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
        s = mem[224]
        t = var305001
        u = var305003
        while _7839 >= 2:
            if uint16(t) > _7839 - 2:
                if var226008 > -u - 1:
                    revert with 'NH{q', 17
                if uint16(var226003) == 65535:
                    revert with 'NH{q', 17
                if mem[mem[160]] < 1:
                    revert with 'NH{q', 17
                if uint16(uint16(var226003) + 1) <= mem[mem[160]] - 1:
                    if uint16(uint16(var226003) + 1) >= mem[mem[160]]:
                        revert with 'NH{q', 50
                    if uint16(uint16(var226003) + 1) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
                    s = mem[224]
                    t = 0
                    u = mem[(32 * uint16(uint16(var226003) + 1)) + mem[160] + 32]
                    continue 
                require var226008 + u >= mem[96]
                mem[mem[64] + 32] = 9
                mem[mem[64] + 64] = 'MAN INJAM'
                emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
                mem[mem[64]] = var226008 + u
                emit 0x2f8e8d68: (var226008 + u)
                if 0 >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if 0 >= mem[mem[224]]:
                    revert with 'NH{q', 50
                if mem[mem[mem[224] + 32]] < 1:
                    revert with 'NH{q', 17
                if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
                    revert with 'NH{q', 50
                if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
                    mem[mem[64] + 4] = msg.sender
                    mem[mem[64] + 36] = var226008 + u
                    require ext_code.size(address(_1749))
                    call address(_1749).transfer(address arg1, uint256 arg2) with:
                         gas gas_remaining wei
                        args msg.sender, var226008 + u
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8224 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_8224] == bool(mem[_8224])
                else:
                    if bool(mem[288]) != 1:
                        mem[mem[64] + 4] = msg.sender
                        mem[mem[64] + 36] = var226008 + u
                        require ext_code.size(address(_1749))
                        call address(_1749).transfer(address arg1, uint256 arg2) with:
                             gas gas_remaining wei
                            args msg.sender, var226008 + u
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8230 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8230] == bool(mem[_8230])
                    else:
                        mem[mem[64] + 32] = 9
                        mem[mem[64] + 64] = 'MAN UNJAM'
                        emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
                        emit 0x2f8e8d68: (var226008 + u)
                        mem[mem[64] + 4] = var226008 + u
                        require ext_code.size(sub_c052d0abAddress)
                        call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                             gas gas_remaining wei
                            args (var226008 + u)
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8278 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8278] == mem[_8278]
                        emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
                        emit 0x2f8e8d68: mem[_8278]
                        call msg.sender with:
                           value eth.balance(this.address) wei
                             gas 2300 * is_zero(value) wei
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                return (var226008 + u)
            if uint16(var226003) >= mem[mem[192]]:
                revert with 'NH{q', 50
            if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[192] + 32]]:
                revert with 'NH{q', 50
            if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] > 7:
                if uint16(var226003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                if uint16(var226003) >= mem[mem[192]]:
                    revert with 'NH{q', 50
                if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[192] + 32]]:
                    revert with 'NH{q', 50
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] != 8:
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] != 9:
                        if uint16(t) == 65535:
                            revert with 'NH{q', 17
                        if uint16(var226003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
                        s = mem[224]
                        t = uint16(t) + 1
                        u = u
                        continue 
                    if uint16(var226003) >= mem[mem[192]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[192] + 32]]:
                        revert with 'NH{q', 50
                    _7939 = mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 32]
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                        revert with 'NH{q', 50
                    mem[0] = 13
                    idx = 0
                    while idx <= 2:
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7939)])
                        call sub_a5218fd8[uint16(_7939)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        _8364 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8364] == mem[_8364 + 12 len 20]
                        if uint16(var226003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        mem[mem[64] + 4] = idx
                        require ext_code.size(sub_a5218fd8[uint16(_7939)])
                        call sub_a5218fd8[uint16(_7939)].coins(uint256 arg1) with:
                             gas gas_remaining wei
                            args idx
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[224] + 32] + 44 len 20] != mem[_8364 + 12 len 20]:
                            _8465 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8465] == mem[_8465 + 12 len 20]
                        else:
                            _8471 = mem[64]
                            mem[64] = mem[64] + ceil32(return_data.size)
                            require return_data.size >= 32
                            require mem[_8471] == mem[_8471 + 12 len 20]
                        if uint16(var226003) >= mem[mem[224]]:
                            revert with 'NH{q', 50
                        if uint16(t) > 65534:
                            revert with 'NH{q', 17
                        if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                            revert with 'NH{q', 50
                        if idx == -1:
                            revert with 'NH{q', 17
                        _7839 = mem[mem[(32 * uint16(var226003)) + s + 32]]
                        idx = idx + 1
                        continue 
                    mem[mem[64] + 4] = 0
                    mem[mem[64] + 36] = 0
                    mem[mem[64] + 68] = u
                    mem[mem[64] + 100] = 1
                    require ext_code.size(sub_a5218fd8[uint16(_7939)])
                    call sub_a5218fd8[uint16(_7939)].0xe95dc522 with:
                         gas gas_remaining wei
                        args 0, 0, u, 1
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8427 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_8427] == mem[_8427]
                    if uint16(t) == 65535:
                        revert with 'NH{q', 17
                    if uint16(var226003) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
                    s = mem[224]
                    t = uint16(t) + 1
                    u = mem[_8427]
                    continue 
                _7906 = mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 32]
                if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                    revert with 'NH{q', 50
                mem[0] = 13
                idx = 0
                while idx <= 2:
                    mem[mem[64] + 4] = idx
                    require ext_code.size(sub_a5218fd8[uint16(_7906)])
                    call sub_a5218fd8[uint16(_7906)].coins(uint256 arg1) with:
                         gas gas_remaining wei
                        args idx
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    _8366 = mem[64]
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_8366] == mem[_8366 + 12 len 20]
                    if uint16(var226003) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                        revert with 'NH{q', 50
                    mem[mem[64] + 4] = idx
                    require ext_code.size(sub_a5218fd8[uint16(_7906)])
                    call sub_a5218fd8[uint16(_7906)].coins(uint256 arg1) with:
                         gas gas_remaining wei
                        args idx
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:
                        revert with ext_call.return_data[0 len return_data.size]
                    if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[224] + 32] + 44 len 20] != mem[_8366 + 12 len 20]:
                        _8466 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8466] == mem[_8466 + 12 len 20]
                    else:
                        _8472 = mem[64]
                        mem[64] = mem[64] + ceil32(return_data.size)
                        require return_data.size >= 32
                        require mem[_8472] == mem[_8472 + 12 len 20]
                    if uint16(var226003) >= mem[mem[224]]:
                        revert with 'NH{q', 50
                    if uint16(t) > 65534:
                        revert with 'NH{q', 17
                    if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                        revert with 'NH{q', 50
                    if idx == -1:
                        revert with 'NH{q', 17
                    _7839 = mem[mem[(32 * uint16(var226003)) + s + 32]]
                    idx = idx + 1
                    continue 
                mem[mem[64] + 4] = 0
                require ext_code.size(sub_a5218fd8[uint16(_7906)])
                staticcall sub_a5218fd8[uint16(_7906)].balances(uint256 arg1) with:
                        gas gas_remaining wei
                       args 0
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _8371 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                _8383 = mem[_8371]
                require mem[_8371] == mem[_8371]
                mem[mem[64] + 36] = 0
                mem[mem[64] + 68] = u
                mem[mem[64] + 100] = 1
                require ext_code.size(sub_a5218fd8[uint16(_7906)])
                call sub_a5218fd8[uint16(_7906)].0xe95dc522 with:
                     gas gas_remaining wei
                    args 0, 0, u, 1
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                mem[mem[64] + 4] = 0
                require ext_code.size(sub_a5218fd8[uint16(_7906)])
                staticcall sub_a5218fd8[uint16(_7906)].balances(uint256 arg1) with:
                        gas gas_remaining wei
                       args 0
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                _8566 = mem[64]
                mem[64] = mem[64] + ceil32(return_data.size)
                require return_data.size >= 32
                require mem[_8566] == mem[_8566]
                if _8383 < mem[_8566]:
                    revert with 'NH{q', 17
                if uint16(t) == 65535:
                    revert with 'NH{q', 17
                if uint16(var226003) >= mem[mem[224]]:
                    revert with 'NH{q', 50
                _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
                s = mem[224]
                t = uint16(t) + 1
                u = _8383 - mem[_8566]
                continue 
            _7872 = mem[64]
            mem[mem[64] + 32 len 64] = call.data[calldata.size len 64]
            mem[mem[64] + 96] = 2
            mem[64] = mem[64] + 192
            mem[_7872 + 128 len 64] = call.data[calldata.size len 64]
            if uint16(var226003) >= mem[mem[224]]:
                revert with 'NH{q', 50
            if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                revert with 'NH{q', 50
            if 0 >= mem[_7872 + 96]:
                revert with 'NH{q', 50
            mem[_7872 + 128] = mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[224] + 32] + 44 len 20]
            if uint16(var226003) >= mem[mem[224]]:
                revert with 'NH{q', 50
            if uint16(t) > 65534:
                revert with 'NH{q', 17
            if uint16(uint16(t) + 1) >= mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]:
                revert with 'NH{q', 50
            if 1 >= mem[_7872 + 96]:
                revert with 'NH{q', 50
            mem[_7872 + 160] = mem[(32 * uint16(uint16(t) + 1)) + mem[(32 * uint16(var226003)) + mem[224] + 32] + 44 len 20]
            if uint16(var226003) >= mem[mem[192]]:
                revert with 'NH{q', 50
            if uint16(t) >= mem[mem[(32 * uint16(var226003)) + mem[192] + 32]]:
                revert with 'NH{q', 50
            _8032 = mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 32]
            if mem[(32 * uint16(t)) + mem[(32 * uint16(var226003)) + mem[192] + 32] + 62 len 2] >= sub_a5218fd8.length:
                revert with 'NH{q', 50
            mem[0] = 13
            if 3600 > -block.timestamp - 1:
                revert with 'NH{q', 17
            mem[_7872 + 192] = 0x38ed173900000000000000000000000000000000000000000000000000000000
            mem[_7872 + 196] = u
            mem[_7872 + 228] = 1
            mem[_7872 + 260] = 160
            mem[_7872 + 356] = mem[_7872 + 96]
            idx = 0
            u = _7872 + 128
            v = _7872 + 388
            while idx < mem[_7872 + 96]:
                mem[v] = mem[u + 12 len 20]
                _7839 = mem[mem[(32 * uint16(var226003)) + s + 32]]
                idx = idx + 1
                u = u + 32
                v = v + 32
                continue 
            mem[_7872 + 292] = this.address
            mem[_7872 + 324] = block.timestamp + 3600
            require ext_code.size(sub_a5218fd8[uint16(_8032)])
            call sub_a5218fd8[uint16(_8032)].mem[mem[64] len 4] with:
                 gas gas_remaining wei
                args mem[mem[64] + 4 len _7872 + (32 * mem[_7872 + 96]) + -mem[64] + 384]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _8386 = mem[64]
            mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
            mem[64] = mem[64] + ceil32(return_data.size)
            require return_data.size >= 32
            _8389 = mem[_8386]
            require mem[_8386] <= test266151307()
            require _8386 + mem[_8386] + 31 < _8386 + return_data.size
            _8392 = mem[_8386 + mem[_8386]]
            if mem[_8386 + mem[_8386]] > test266151307():
                revert with 'NH{q', 65
            if _8386 + ceil32(return_data.size) + floor32(mem[_8386 + mem[_8386]]) + 1 > test266151307() or floor32(mem[_8386 + mem[_8386]]) + 1 < 0:
                revert with 'NH{q', 65
            mem[64] = _8386 + ceil32(return_data.size) + floor32(mem[_8386 + mem[_8386]]) + 1
            mem[_8386 + ceil32(return_data.size)] = _8392
            require _8389 + (32 * _8392) + 32 <= return_data.size
            idx = 0
            u = _8386 + _8389 + 32
            v = _8386 + ceil32(return_data.size) + 32
            while idx < _8392:
                require mem[u] == mem[u]
                mem[v] = mem[u]
                _7839 = mem[mem[(32 * uint16(var226003)) + s + 32]]
                idx = idx + 1
                u = u + 32
                v = v + 32
                continue 
            if 1 >= _8392:
                revert with 'NH{q', 50
            if uint16(t) == 65535:
                revert with 'NH{q', 17
            if uint16(var226003) >= mem[mem[224]]:
                revert with 'NH{q', 50
            _7839 = mem[mem[(32 * uint16(var226003)) + mem[224] + 32]]
            s = mem[224]
            t = uint16(t) + 1
            u = mem[_8386 + ceil32(return_data.size) + 64]
            continue 
        revert with 'NH{q', 17
    require var222007 >= mem[96]
    mem[mem[64] + 32] = 9
    mem[mem[64] + 64] = 'MAN INJAM'
    emit 0xf383fc9b: Array(len=9, data='MAN INJAM')
    mem[mem[64]] = var222007
    emit 0x2f8e8d68: var222007
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if 0 >= mem[mem[224]]:
        revert with 'NH{q', 50
    if mem[mem[mem[224] + 32]] < 1:
        revert with 'NH{q', 17
    if mem[mem[mem[224] + 32]] - 1 >= mem[mem[mem[224] + 32]]:
        revert with 'NH{q', 50
    if mem[(32 * mem[mem[mem[224] + 32]] - 1) + mem[mem[224] + 32] + 44 len 20] != sub_b6a21b61Address:
        mem[mem[64] + 4] = msg.sender
        mem[mem[64] + 36] = var222007
        require ext_code.size(address(_1749))
        call address(_1749).transfer(address arg1, uint256 arg2) with:
             gas gas_remaining wei
            args msg.sender, var222007
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _5979 = mem[64]
        mem[64] = mem[64] + ceil32(return_data.size)
        require return_data.size >= 32
        require mem[_5979] == bool(mem[_5979])
    else:
        if bool(('cd', 4)[5]) != 1:
            mem[mem[64] + 4] = msg.sender
            mem[mem[64] + 36] = var222007
            require ext_code.size(address(_1749))
            call address(_1749).transfer(address arg1, uint256 arg2) with:
                 gas gas_remaining wei
                args msg.sender, var222007
            mem[mem[64]] = ext_call.return_data[0]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _5988 = mem[64]
            mem[64] = mem[64] + ceil32(return_data.size)
            require return_data.size >= 32
            require mem[_5988] == bool(mem[_5988])
        else:
            mem[mem[64] + 32] = 9
            mem[mem[64] + 64] = 'MAN UNJAM'
            emit 0xf383fc9b: Array(len=9, data='MAN UNJAM')
            emit 0x2f8e8d68: var222007
            mem[mem[64] + 4] = var222007
            require ext_code.size(sub_c052d0abAddress)
            call sub_c052d0abAddress.withdraw(uint256 arg1) with:
                 gas gas_remaining wei
                args var222007
            mem[mem[64]] = ext_call.return_data[0]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _6071 = mem[64]
            mem[64] = mem[64] + ceil32(return_data.size)
            require return_data.size >= 32
            require mem[_6071] == mem[_6071]
            emit 0xf383fc9b: Array(len=9, data='MAN ANJAM')
            emit 0x2f8e8d68: mem[_6071]
            call msg.sender with:
               value eth.balance(this.address) wei
                 gas 2300 * is_zero(value) wei
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
    return var222007
}



}
