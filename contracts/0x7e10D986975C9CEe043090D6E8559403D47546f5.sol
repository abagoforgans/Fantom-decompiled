...
mapping of uint256 sub_bcb6e48b;
...
function sub_bcb6e48b(?) {
    require calldata.size - 4 >= 32
    return sub_bcb6e48b[arg1]
}
...
function notifyRewardAmount(uint256 arg1) {
    ...
    if not totalSupply:
        ...
        if block.timestamp >= periodFinish:
            rewardRate = arg1 / 1134420
            lastUpdateTime = block.timestamp
            if block.timestamp + 1134420 < block.timestamp:
                revert with 0, 'SafeMath: addition overflow'
            periodFinish = block.timestamp + 1134420
            ...
            if not periodFinish - block.timestamp:
                ...
                if arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                if arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) / 1134420 < rewardRate:
                    revert with 0, 'New reward rate too low'
                rewardRate = arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) / 1134420
            lastUpdateTime = block.timestamp
            if block.timestamp + 1134420 < block.timestamp:
                revert with 0, 'SafeMath: addition overflow'
            periodFinish = block.timestamp + 1134420
...
function withdraw(uint256 arg1) {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            ...
            if sub_bcb6e48b[address(msg.sender)] + (168 * 24 * 3600) >= block.timestamp:
