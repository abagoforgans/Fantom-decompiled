...
function changeFees(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    fees = arg1
...
function createNew(string arg1) payable {
    require calldata.size - 4 >= 32
    require arg1 <= 4294967296
    require arg1 + 36 <= calldata.size
    require arg1.length <= 4294967296 and arg1 + arg1.length + 36 <= calldata.size
    ...
    if ext_call.return_data[0] < fees:
        revert with 0, 'Allowance is below fees'
    require ext_code.size(sub_5d33a1f4Address)
    call sub_5d33a1f4Address.0x23b872dd with:
         gas gas_remaining wei
        args msg.sender, feeReceiverAddress, fees
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    stor11++
    ...
    ('bool', ('stor', ('map', ('stor', ('name', 'stor11', 11)), ('name', 'stor3', 3))))
    if arg1.length:
        stor8[stor11][] = Array(len=arg1.length, data=arg1[all])
    else:
        stor8[stor11] = 0
        idx = 0
        while stor8[stor11].length + 31 / 32 > idx:
