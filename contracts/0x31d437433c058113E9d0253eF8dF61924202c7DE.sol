    ...
    if not return_data.size:
        ...
        if sub_0fc99376 == stor9:
            ...
            while (32 * players.length) + 388 > idx + 32:
                ...
                continue 
            require players.length
            require sha3(block.number, block.timestamp, mem[388 len 32 * players.length]) % players.length < players.length
            ...
            call sub_b1b0c49eAddress.0xa9059cbb with:
                 gas gas_remaining wei
                args players[sha3(block.number, block.timestamp, mem[388 len 32 * players.length]) % players.length].field_0, ext_call.return_data[0]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            require return_data.size >= 32
            require sha3(block.number, block.timestamp, mem[388 len 32 * players.length]) % players.length < players.length
            if not players[sha3(block.number, block.timestamp, mem[388 len 32 * players.length]) % players.length].field_0:
                revert with 0, 'ERC20: mint to the zero address'
            if totalSupply + 1 < totalSupply:
                revert with 0, 'SafeMath: addition overflow'
            totalSupply++
            if balanceOf[stor7[sha3(block.number, block.timestamp, mem[388 len 32 * stor7.length]) % stor7.length].field_0] + 1 < balanceOf[stor7[sha3(block.number, block.timestamp, mem[388 len 32 * stor7.length]) % stor7.length].field_0]:
                revert with 0, 'SafeMath: addition overflow'
            balanceOf[stor7[sha3(block.number, block.timestamp, mem[388 len 32 * stor7.length]) % stor7.length].field_0]++
            emit Transfer(1, 0, players[sha3(block.number, block.timestamp, mem[388 len 32 * players.length]) % players.length].field_0);
            players.length = 0
            idx = 0
