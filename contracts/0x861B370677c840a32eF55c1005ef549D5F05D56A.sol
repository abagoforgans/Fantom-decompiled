...
function distribute() payable {
    ...
    if eth.balance(this.address) < 10^18:
        revert with 0, 17
    if eth.balance(this.address) - 10^18 and stor2 > -1 / eth.balance(this.address) - 10^18:
        revert with 0, 17
    call stor1 with:
       value (-1 * 10^18 * stor2) + (eth.balance(this.address) * stor2) / 100 wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    if eth.balance(this.address) - 10^18 < (-1 * 10^18 * stor2) + (eth.balance(this.address) * stor2) / 100:
        revert with 0, 17
    require ext_code.size(stor3)
    call stor3.reflectToOwners() with:
       value eth.balance(this.address) + -((-1 * 10^18 * stor2) + (eth.balance(this.address) * stor2) / 100) - 10^18 wei
         gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
