...
function sub_a2e3cd84(?) payable {
    ...
    if arg1 == this.address:
        revert with 0, 'Cannot withdraw self'
    if 0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83 == this.address:
        revert with 0, 'Cannot withdraw user tokens'
            ...
            if block.timestamp <= bonusEndTimestamp:
                ...
                if not block.timestamp - uint256(poolInfo.field_512):
                    ...
                    if (block.timestamp * rewardPerSec) - (uint256(poolInfo.field_512) * rewardPerSec):
                        ...
                        if not stor6:
                            revert with 0, 'SafeMath: division by zero'
                        if (block.timestamp * rewardPerSec * uint256(poolInfo.field_256)) - (uint256(poolInfo.field_512) * rewardPerSec * uint256(poolInfo.field_256)) / stor6:
                            if 10^12 * (block.timestamp * rewardPerSec * uint256(poolInfo.field_256)) - (uint256(poolInfo.field_512) * rewardPerSec * uint256(poolInfo.field_256)) / stor6 / (block.timestamp * rewardPerSec * uint256(poolInfo.field_256)) - (uint256(poolInfo.field_512) * rewardPerSec * uint256(poolInfo.field_256)) / stor6 != 10^12:
                                revert with 0, 32, 33, 0x73536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[325 len 31]
                            if not ext_call.return_data[0]:
                                revert with 0, 'SafeMath: division by zero'
