...
function sub_f3a165fe(?) payable {
    ...
    while idx < arg2:
        ...
        if not ownerOf[_171]:
            revert with 0, 'ERC721: owner query for nonexistent token'
        if msg.sender == ownerOf[_171]:
            if not ownerOf[_171]:
                revert with 0, 'ERC721: owner query for nonexistent token'
            if ownerOf[_171] != this.address:
                revert with 0, 'ERC721: transfer of token that is not own'
            if not msg.sender:
                revert with 0, 'ERC721: transfer to the zero address'
            approved[_171] = 0
            if not ownerOf[_171]:
                revert with 0, 'ERC721: owner query for nonexistent token'
            emit Approval(ownerOf[_171], 0, _171);
            ...
            mem[0] = _171
            mem[32] = 0
            ownerOf[_171] = msg.sender
            emit Transfer(this.address, msg.sender, _171);
            if ext_code.size(msg.sender):
                ...
                s = 0
                while s < 0:
                    mem[s + mem[64] + 164] = mem[s + _174 + 32]
                    s = s + 32
                    continue 
                require ext_code.size(msg.sender)
                call msg.sender.0x150b7a02 with:
                     gas gas_remaining wei
                    args msg.sender, address(this.address), _171, 128, 0
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:
                    if not return_data.size:
                        if not mem[96]:
            ...
            if approved[_171] == msg.sender:
                ...
                if ext_code.size(msg.sender):
                    ...
                    mem[64] = mem[64] + ceil32(return_data.size)
                    require return_data.size >= 32
                    require mem[_343] == Mask(32, 224, mem[_343])
                    if Mask(32, 224, mem[_343]) != 0x150b7a0200000000000000000000000000000000000000000000000000000000:
                        revert with 0, 'ERC721: transfer to non ERC721Receiver implementer'
            else:
                if not stor3[stor0[_171]][address(msg.sender)]:
                    revert with 0, 'ERC721: transfer caller is not owner nor approved'
                if not ownerOf[_171]:
                    revert with 0, 'ERC721: owner query for nonexistent token'
                if ownerOf[_171] != this.address:
