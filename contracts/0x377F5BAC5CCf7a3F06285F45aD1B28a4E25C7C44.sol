...
address sub_3f12e187Address;
uint256 totalSupply;
mapping of uint256 balanceOf;
address owner;
address stor4;
address sub_6cb3b1bdAddress;
uint256 initreward;
uint256 starttime;
...
function sub_3f12e187(?) payable {
    return sub_3f12e187Address
}

function sub_6cb3b1bd(?) payable {
    return sub_6cb3b1bdAddress
}
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                rewardRate = arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) / 168 * 24 * 3600
    require ext_code.size(sub_6cb3b1bdAddress)
    call sub_6cb3b1bdAddress.0x40c10f19 with:
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            if block.timestamp < periodFinish:
                ...
                if not ext_code.hash(sub_3f12e187Address):
                    revert with 0, 'SafeERC20: call to non-contract'
                if ext_code.hash(sub_3f12e187Address) == 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470:
                    revert with 0, 'SafeERC20: call to non-contract'
                mem[228 len 96] = unknown_0x23b872dd(?????), msg.sender, address(this.address), Mask(224, 32, arg1) >> 32
                mem[352 len 4] = uint32(arg1)
                call sub_3f12e187Address with:
                     gas gas_remaining wei
                ...
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                rewardRate = initreward / 168 * 24 * 3600
                if block.timestamp + (168 * 24 * 3600) < block.timestamp:
                    revert with 0, 'SafeMath: addition overflow'
                periodFinish = block.timestamp + (168 * 24 * 3600)
