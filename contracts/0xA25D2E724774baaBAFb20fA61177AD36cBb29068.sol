...
function migrate(address arg1) payable {
    ...
    if stor11 > -2001:
        revert with 'NH{q', 17
    if stor11 + 2000 <= userCount:
        idx = stor11
        while idx < stor11 + 2000:
            mem[0] = users[idx]
            mem[32] = 4
...
function attachValue(bytes arg1) payable {
    ...
    if stor14[ext_call.return_data[0]]:
        revert with 0, 'Data is not processed.'
    stor14[ext_call.return_data[0]] = 1
    emit Transfer(address(ext_call.return_data[0]), address(ext_call.return_data[0]), ext_call.return_data[0], ext_call.return_data[0]);
    if not stor3[address(ext_call.return_data[0])]:
        revert with 0, 'this token is not supported by treasury'
