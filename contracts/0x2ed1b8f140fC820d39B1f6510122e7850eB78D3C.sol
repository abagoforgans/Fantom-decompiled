...
function sub_b54d51ba(?) payable {
    ...
    while idx < ('cd', 4).length:
        require ext_code.size(sub_b4f6f509Address)
        staticcall sub_b4f6f509Address.next_summoner() with:
                gas gas_remaining wei
        ...
        sub_d38b576d.length++
        mem[0] = 2
        sub_d38b576d[sub_d38b576d.length] = _26
        if idx >= mem[96]:
            revert with 'NH{q', 50
        _29 = mem[(32 * idx) + 128]
        ...
        mem[mem[64] + 4] = _29
        require ext_code.size(sub_b4f6f509Address)
        call sub_b4f6f509Address.summon(uint256 arg1) with:
             gas gas_remaining wei
            args _29
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
