...
function createPair(address arg1, address arg2) payable {
    ...
    if arg1 < arg2:
        ...
        create contract with 0 wei
                        code: code.data[1493 len 9176]
        if not create.new_address:
            revert with ext_call.return_data[0 len return_data.size]
        require ext_code.size(address(create.new_address))
        call address(create.new_address).initialize(address rg1, address rg2) with:
             gas gas_remaining wei
            args address(arg1), arg2
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        pair[address(arg1)][address(arg2)] = address(create.new_address)
        pair[address(arg2)][address(arg1)] = address(create.new_address)
        allPairs.length++
        allPairs[allPairs.length] = address(create.new_address)
        emit PairCreated(address(create.new_address), allPairs.length, arg1, arg2);
    else:
        if not arg2:
            revert with 0, 'UniswapV2: ZERO_ADDRESS'
        ...
        allPairs.length++
        allPairs[allPairs.length] = address(create.new_address)
        emit PairCreated(address(create.new_address), allPairs.length, arg2, arg1);
    return 0
}
