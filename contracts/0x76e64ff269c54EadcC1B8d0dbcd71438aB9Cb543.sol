...
function sub_a871c915(?) payable {
    if characters.length > test266151307():
        revert with 0, 65
    mem[96] = characters.length
    mem[64] = (32 * characters.length) + 128
    if not characters.length:
        idx = 0
        while idx < characters.length:
            mem[0] = 1
            mem[mem[64] + 4] = characters[idx]
            ...
            if idx >= mem[96]:
                revert with 0, 50
            if mem[_37] + (24 * 3600) > block.timestamp:
                mem[(32 * idx) + 128] = 1
            else:
                mem[(32 * idx) + 128] = 0
            if idx == -1:
        ...
        return memory
          from mem[64]
           len _24 + (32 * _30) + -mem[64] + 64
    mem[128 len 32 * characters.length] = call.data[calldata.size len 32 * characters.length]
    idx = 0
    while idx < characters.length:
        mem[0] = 1
        mem[mem[64] + 4] = characters[idx]
        ...
        if idx >= mem[96]:
            revert with 0, 50
        if mem[_39] + (24 * 3600) > block.timestamp:
            mem[(32 * idx) + 128] = 1
        else:
            mem[(32 * idx) + 128] = 0
        if idx == -1:
