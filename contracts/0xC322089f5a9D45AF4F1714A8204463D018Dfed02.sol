...
function sub_80572309(?) payable {
    ...
    if not 10^ext_call.return_data[0]:
        revert with 0, 'SafeMath: division by zero'
    return (arg2 * ext_call.return_data[0] / 10^ext_call.return_data[0])
}
...
function getUserAccountData(address arg1) payable {
    ...
    require return_data.size >= 32
    require ext_code.size(address(ext_call.return_data[0]))
    staticcall address(ext_call.return_data[0]).0xe563a7d0 with:
            gas gas_remaining wei
           args msg.sender, 0
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
