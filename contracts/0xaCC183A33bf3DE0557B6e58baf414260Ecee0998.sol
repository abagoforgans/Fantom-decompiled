...
function sub_515e09a6(?) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'ACW'
    shares.length++
    address(shares[shares.length]) = address(arg1)
    farms.length++
    address(farms[farms.length]) = address(arg2)
    lastPortions.length++
    lastPortions[lastPortions.length] = arg3
    emit 0x73e953c6: farms.length, arg3, address(arg1), address(arg2)
}
            ...
            if currentUser + arg1 <= totalUsers:
                if totalShares > 0:
                    ...
                    while idx < currentUser + arg1:
                        ...
                        staticcall address(shares[stor5]).userIdByIndex(uint256 arg1) with:
