...
function sub_8649fe86(?) payable {
    mem[64] = 96
    if var91002 >= sub_4a2fc9e8.length:
    if var97001 >= sub_4a2fc9e8.length:
        revert with 'NH{q', 50
    mem[0] = 3
    mem[96] = 0x5c58df200000000000000000000000000000000000000000000000000000000
    mem[var111003] = var111001
    require ext_code.size(rarityAddress)
    staticcall rarityAddress.mem[var117003 len 4] with:
            gas gas_remaining wei
...
function sub_3a845c3b(?) payable {
    ...
    while idx < ext_call.return_data[0]:
        ...
        mem[mem[64] + 36] = 0x53be63c557751f9a3c33c78a2f270efc45fce959
        mem[mem[64] + 68] = _20
        require ext_code.size(rarityAddress)
        call rarityAddress.0x42842e0e with:
             gas gas_remaining wei
            args address(arg1), 0x53be63c557751f9a3c33c78a2f270efc45fce959, _20
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        if idx == -1:
            revert with 'NH{q', 17
