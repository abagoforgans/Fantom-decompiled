...
#  - verifyUniAddress(address arg1, address arg2)
#
...
function isUniswapPair(address arg1) payable {
    ...
    if not return_data.size:
        ...
        mem[200 len return_data.size] = ext_call.return_data[0 len return_data.size]
        if not ext_call.success:
            return 0
        if not ext_call.success:
            return 0
        if 0 == this.address:
            return True
        mem[ceil32(return_data.size) + 169] = mem[212 len 20] == this.address
        return memory
          from ceil32(return_data.size) + 169
           len 32
    mem[132] = return_data.size
    mem[164 len return_data.size] = ext_call.return_data[0 len return_data.size]
    mem[ceil32(return_data.size) + 133] = 4
    mem[ceil32(return_data.size) + 165 len 4] = token1()
    mem[ceil32(return_data.size) + 169] = 0, mem[ceil32(return_data.size) + 169 len 28]
    mem[ceil32(return_data.size) + 173] = 0
    call arg1.mem[ceil32(return_data.size) + 169 len 4] with:
         gas gas_remaining wei
    if not return_data.size:
        if not ext_call.success:
            return 0
        if not ext_call.success:
            return 0
        if mem[176 len 20] != this.address:
            return (0 == this.address)
        mem[ceil32(return_data.size) + 169] = mem[176 len 20] == this.address
        return memory
          from ceil32(return_data.size) + 169
           len 32
    mem[ceil32(return_data.size) + 169] = return_data.size
    ...
    if not ext_call.success:
        return 0
    if mem[176 len 20] == this.address:
        mem[(2 * ceil32(return_data.size)) + 170] = mem[176 len 20] == this.address
    else:
        mem[(2 * ceil32(return_data.size)) + 170] = mem[ceil32(return_data.size) + 213 len 20] == this.address
    return memory
      from (2 * ceil32(return_data.size)) + 170
       len 32
}
