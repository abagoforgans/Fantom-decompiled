...
function sub_3c21fbcb(?) payable {
    return sub_3c21fbcbAddress
}
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if (periodFinish * rewardRate) - (block.timestamp * rewardRate) + arg1 < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                rewardRate = (periodFinish * rewardRate) - (block.timestamp * rewardRate) + arg1 / 24 * 3600
    else:
        if block.timestamp < periodFinish:
            if lastUpdateTime > block.timestamp:
                revert with 0, 'SafeMath: subtraction overflow'
...
function stake(uint256 arg1) payable {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            ...
            if 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 == ext_code.hash(sub_3c21fbcbAddress):
                revert with 0, 'SafeERC20: call to non-contract'
            if not ext_code.hash(sub_3c21fbcbAddress):
                revert with 0, 'SafeERC20: call to non-contract'
            mem[228 len 96] = unknown_0x23b872dd(?????), msg.sender, address(this.address), Mask(224, 32, arg1) >> 32
            mem[352 len 4] = uint32(arg1)
            call sub_3c21fbcbAddress with:
