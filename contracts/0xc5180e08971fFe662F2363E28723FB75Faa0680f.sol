...
function sub_24df1118(?) payable {
    ...
    while idx < arg2:
        ...
        call sub_d486adccAddress.0x379607f5 with:
             gas gas_remaining wei
            args sub_284b91bd[idx]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
...
function sub_2f4e8d0b(?) payable {
    ...
    while idx < arg3:
        ...
        call sub_d486adccAddress.transfer(uint256 arg1, uint256 arg2, uint256 arg3) with:
             gas gas_remaining wei
            args sub_284b91bd[idx], arg1, _15
        mem[mem[64]] = ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _19 = mem[64]
...
function sub_b89e6dbc(?) payable {
    ...
    while idx < arg3:
        ...
        call rarityAddress.0x42842e0e with:
             gas gas_remaining wei
            args address(arg1), this.address, _20
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        sub_284b91bd.length++
        mem[0] = 3
        sub_284b91bd[sub_284b91bd.length] = _20
        if sub_dfad1b51 == -1:
            revert with 0, 17
        sub_dfad1b51++
        if idx == -1:
