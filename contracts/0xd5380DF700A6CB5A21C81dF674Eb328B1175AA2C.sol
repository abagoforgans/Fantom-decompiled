...
function sub_a7ccc94b(?) payable {
    ...
    require _218 + mem[_218] + 31 < _218 + return_data.size
    _220 = mem[_218 + mem[_218]]
    if mem[_218 + mem[_218]] > test266151307():
        revert with 'NH{q', 65
    if _218 + ceil32(return_data.size) + floor32(mem[_218 + mem[_218]]) + 1 > test266151307() or floor32(mem[_218 + mem[_218]]) + 1 < 0:
        revert with 'NH{q', 65
    mem[64] = _218 + ceil32(return_data.size) + floor32(mem[_218 + mem[_218]]) + 1
    mem[_218 + ceil32(return_data.size)] = _220
    require _219 + (32 * _220) + 32 <= return_data.size
    idx = 0
    s = _218 + _219 + 32
    t = _218 + ceil32(return_data.size) + 32
    while idx < _220:
        require mem[s] == mem[s]
        mem[t] = mem[s]
        idx = idx + 1
    ...
    mem[mem[64] + 36] = _250
    require ext_code.size(address(_246))
    call address(_246).approve(address rg1, uint256 rg2) with:
         gas gas_remaining wei
        args address(arg2), _250
    mem[mem[64]] = ext_call.return_data[0]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    _253 = mem[64]
    ...
    call address(arg2).swapExactTokensForETH(uint256 rg1, uint256 rg2, address[] rg3, address rg4, uint256 rg5) with:
         gas gas_remaining wei
        args _250, 0, 160, msg.sender, block.timestamp, mem[mem[64] + 164 len (32 * _258) + 32]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    _268 = mem[64]
    mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
    ...
    mem[_268 + ceil32(return_data.size)] = _270
