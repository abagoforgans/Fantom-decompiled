...
function forceFileClaim(address arg1, bytes32 arg2, uint48 arg3) payable {
    ...
    if not ext_code.hash(feeCurrencyAddress):
        revert with 0, 'Address: call to non-contract'
    mem[(4 * ceil32(return_data.size)) + 740 len 128] = transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), forceClaimFee, 0
    mem[(4 * ceil32(return_data.size)) + 840] = 0
    call feeCurrencyAddress with:
       funct Mask(32, 224, transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), forceClaimFee, 0) >> 224
         gas gas_remaining wei
        args (Mask(768, -544, transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), forceClaimFee, 0) << 544)
    if not return_data.size:
        if not ext_call.success:
            if ext_call.return_data[0]:
                revert with memory
