...
function sub_e157054a(?) payable {
    ...
    while idx < ('cd', 4).length:
        ...
        call stor3.0x42842e0e with:
             gas gas_remaining wei
            args this.address, msg.sender, _24
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        if idx == -1:
            revert with 0, 17
...
function sub_0fb7f998(?) payable {
    ...
    if ('cd', 36).length > 100:
        revert with 0, '!len'
    if fee and ('cd', 36).length > -1 / fee:
        revert with 0, 17
    if msg.value < fee * ('cd', 36).length:
        revert with 0, '!msg.value'
    idx = 0
    while idx < ('cd', 36).length:
        ...
        call stor3.0x42842e0e with:
             gas gas_remaining wei
            args this.address, address(cd[4]), _24
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        if idx == -1:
            revert with 0, 17
