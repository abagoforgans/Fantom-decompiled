                ...
                if not token0ToEarnedPath.length:
                    ...
                    if earnedAddress != token1Address:
                        if ext_call.return_data[0] > 0:
                            ...
                            if not ext_code.size(token1Address):
                                revert with 0, 'Address: call to non-contract'
                            mem[424 len 64] = approve(address arg1, uint256 arg2), Mask(224, 0, stor8), uint32(stor8), Mask(224, 31, ext_call.return_data[0]) >> 31
                            call token1Address with:
                    ...
                    if not token0ToEarnedPath.length:
                        ...
                        if earnedAddress != token1Address:
                            if ext_call.return_data[0] > 0:
                                ...
                                if not ext_code.size(token1Address):
                                    revert with 0, 'Address: call to non-contract'
                                mem[ceil32(return_data.size) + 425 len 64] = approve(address arg1, uint256 arg2), Mask(224, 0, stor8), uint32(stor8), Mask(224, 31, ext_call.return_data[0]) >> 31
                                call token1Address with:
                                   funct token0ToEarnedPath.length.field_224
                                     gas gas_remaining wei
