...
function getLatestTimestamp(address arg1) payable {
    ...
    mem[mem[64] + 36] = ceil32(_12) + 96
    _60 = mem[_10]
    mem[_11 + ceil32(_12) + 100] = mem[_10]
    mem[_11 + ceil32(_12) + 132 len ceil32(_60)] = mem[_10 + 32 len ceil32(_60)]
    if ceil32(_60) <= _60:
        require ext_code.size(address(stor1.length))
        staticcall address(stor1.length).getReferenceData(string rg1, string rg2) with:
                gas gas_remaining wei
               args mem[mem[64] + 4 len _11 + ceil32(_12) + ceil32(_60) + -mem[64] + 128]
        mem[mem[64] len 96] = ext_call.return_data[0 len 96]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _110 = mem[64]
...
function getPrice(address arg1) payable {
    ...
    if ceil32(_12) <= _12:
        ...
        _56 = mem[_10]
        mem[_11 + ceil32(_12) + 100] = mem[_10]
        mem[_11 + ceil32(_12) + 132 len ceil32(_56)] = mem[_10 + 32 len ceil32(_56)]
        if ceil32(_56) <= _56:
            require ext_code.size(address(stor1.length))
            staticcall address(stor1.length).getReferenceData(string rg1, string rg2) with:
                    gas gas_remaining wei
                   args mem[mem[64] + 4 len _11 + ceil32(_12) + ceil32(_56) + -mem[64] + 128]
            mem[mem[64] len 96] = ext_call.return_data[0 len 96]
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            _102 = mem[64]
