...
function sub_927f59ba(?) payable {
    ...
    while idx < ('cd', 4).length:
        ...
        if ownerOf[s]:
            revert with 0, 'ERC721: token already minted'
        if balanceOf[address(_96)] > -2:
            revert with 'NH{q', 17
        balanceOf[address(_96)]++
        mem[0] = s
        mem[32] = 2
        ownerOf[s] = address(_96)
        emit Transfer(0, address(_96), s);
        if ext_code.size(address(_96)) > 0:
            ...
            t = 0
            while t < 0:
                mem[mem[64] + t + 164] = mem[_97 + t + 32]
                t = t + 32
                continue 
            require ext_code.size(address(_96))
            call address(_96).onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
                 gas gas_remaining wei
            ...
            require return_data.size >= 32
            require mem[_141] == Mask(32, 224, mem[_141])
            if Mask(32, 224, mem[_141]) != 0x150b7a0200000000000000000000000000000000000000000000000000000000:
                revert with 0, 'ERC721: transfer to non ERC721Receiver implementer'
        if s == -1:
            revert with 'NH{q', 17
        if idx == -1:
