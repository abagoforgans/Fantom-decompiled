contract main {




// =====================  Runtime code  =====================


const getBalance = eth.balance(this.address)


uint256 maxUsers;
address stor2;
uint256 stor3;
uint256 nextUser;
mapping of uint8 stor5;
mapping of address userIds;

function userList(address arg1) {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    return bool(stor5[arg1])
}

function userIds(uint256 arg1) {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    return userIds[arg1]
}

function nextUser() {
    return nextUser
}

function maxUsers() {
    return maxUsers
}

function sub_eae6d4da(?) {
    return bool(stor5[address(msg.sender)])
}

function _fallback() payable {
    revert
}

function withdrawal() {
    require msg.sender == stor2
    call msg.sender with:
       value eth.balance(this.address) wei
         gas gas_remaining wei
    if not ext_call.success:
        revert with 0, 'Failed to send FTM'
}

function register() payable {
    if stor3 != msg.value:
        revert with 0, 'fee is 1 FTM'
    if nextUser >= maxUsers:
        revert with 0, 'Max users reached'
    if stor5[address(msg.sender)]:
        revert with 0, 'User already in our SYSTEM'
    stor5[address(msg.sender)] = 1
    userIds[stor4] = msg.sender
    emit 0x6b1da47e: nextUser
    if nextUser == -1:
        revert with 'NH{q', 17
    nextUser++
}

function getAll() {
    if maxUsers > test266151307():
        revert with 'NH{q', 65
    mem[96] = maxUsers
    mem[64] = (32 * maxUsers) + 128
    if maxUsers:
        mem[128 len 32 * maxUsers] = call.data[calldata.size len 32 * maxUsers]
    idx = 0
    while idx < maxUsers:
        mem[0] = idx
        mem[32] = 6
        if idx >= maxUsers:
            revert with 'NH{q', 50
        mem[(32 * idx) + 128] = userIds[idx]
        if idx == -1:
            revert with 'NH{q', 17
        idx = idx + 1
        continue 
    mem[(32 * maxUsers) + 128] = 32
    mem[(32 * maxUsers) + 160] = maxUsers
    idx = 0
    s = 128
    t = mem[64] + 64
    while idx < maxUsers:
        mem[t] = mem[s + 12 len 20]
        idx = idx + 1
        s = s + 32
        t = t + 32
        continue 
    return memory
      from mem[64]
       len (64 * maxUsers) + -mem[64] + 192
}



}
