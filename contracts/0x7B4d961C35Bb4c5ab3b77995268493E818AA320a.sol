...
function claimReward() payable {
    ...
    if not stor23.length:
        require ext_code.size(address(masterchefAddress))
        call address(masterchefAddress).claim(address arg1, address[] arg2) with:
             gas gas_remaining wei
            args address(this.address), 64, stor23.length
    else:
        mem[196] = address(stor23.field_0)
        idx = 196
        s = 0
        while (32 * stor23.length) + 196 > idx + 32:
        ...
        call address(masterchefAddress).claim(address arg1, address[] arg2) with:
             gas gas_remaining wei
            args address(this.address), Array(len=stor23.length, data=mem[196 len 32 * stor23.length])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
...
function withdraw(uint256 arg1) payable {
    ...
    require ext_code.size(address(masterchefAddress))
    call address(masterchefAddress).0xf3fef3a3 with:
         gas gas_remaining wei
