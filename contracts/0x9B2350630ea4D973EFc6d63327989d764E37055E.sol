...
function transfer(address arg1, uint256 arg2) payable {
    ...
    while idx < stor9.length:
        ...
        if bool(stor8[address(stor9[idx])]) != 1:
            ...
            stor10.length++
            mem[0] = 10
            address(stor10[stor10.length]) = address(stor9[idx])
        if idx == -1:
            revert with 'NH{q', 17
        idx = idx + 1
        continue 
    ...
    if not stor10.length:
        ...
        idx = 0
        while stor10.length > idx:
            uint256(stor9[s]) = uint256(stor10[idx])
            s = s + 1
            idx = idx + 1
            continue 
        idx = stor10.length
    ...
    if not stor10.length:
        revert with 'NH{q', 18
    stor11 = block.difficulty * block.timestamp % stor10.length
    if stor11 >= stor10.length:
        revert with 'NH{q', 50
    stor12 = address(stor10[stor11])
