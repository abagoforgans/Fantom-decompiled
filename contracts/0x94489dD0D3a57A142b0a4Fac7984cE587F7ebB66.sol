...
function sub_e21c5af0(?) payable {
    return sub_e21c5af0Address
}
    ...
    if not totalSupply:
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if (periodFinish * rewardRate) - (block.timestamp * rewardRate) + arg1 < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                rewardRate = (periodFinish * rewardRate) - (block.timestamp * rewardRate) + arg1 / 1440 * 24 * 3600
    else:
        if block.timestamp < periodFinish:
            if lastUpdateTime > block.timestamp:
                revert with 0, 'SafeMath: subtraction overflow'
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                rewardRate = (periodFinish * rewardRate) - (block.timestamp * rewardRate) + arg1 / 1440 * 24 * 3600
    lastUpdateTime = block.timestamp
    if block.timestamp + (1440 * 24 * 3600) < block.timestamp:
        revert with 0, 'SafeMath: addition overflow'
...
function stake(uint256 arg1) payable {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            ...
            if 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 == ext_code.hash(sub_e21c5af0Address):
                revert with 0, 'SafeERC20: call to non-contract'
            if not ext_code.hash(sub_e21c5af0Address):
                revert with 0, 'SafeERC20: call to non-contract'
