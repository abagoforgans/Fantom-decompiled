...
function sub_6ccb05ea(?) payable {
    require calldata.size - 4 >= 32
    require arg1 < sub_6ccb05ea.length
    return sub_6ccb05ea[arg1]
}
...
function balanceOfPool() payable {
    ...
    staticcall masterchefAddress.userInfo(address arg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function deposit() payable {
    ...
    require return_data.size >= 32
    require ext_code.size(masterchefAddress)
    call masterchefAddress.deposit(uint256 arg1) with:
         gas gas_remaining wei
...
function retireStrat() payable {
    if address(vaultAddress) != msg.sender:
        revert with 0, '!vault'
    require ext_code.size(masterchefAddress)
    call masterchefAddress.emergencyWithdraw() with:
         gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(wantAddress)
