...
function executeDailyLimit(address arg1, address arg2, uint256 arg3) payable {
    ...
    staticcall managerAddress.isOwner(address rg1) with:
    ...
    if arg1:
        call managerAddress.execTransactionFromModule(address rg1, uint256 rg2, bytes rg3, uint8 rg4) with:
             gas gas_remaining wei
            args address(arg1), 0, 128, 0, 68, transfer(address rg1, uint256 rg2), address(arg2) << 64, 0, arg3, 0, address(arg1) << 32 >> 256, 0
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        if not ext_call.return_data[0]:
