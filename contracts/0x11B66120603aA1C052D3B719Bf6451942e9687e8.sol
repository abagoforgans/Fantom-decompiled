...
function sub_423798c3(?) payable {
    ...
    while idx < cd[132]:
        ...
        call stor2.swapExactTokensForTokens(uint256 rg1, uint256 rg2, address[] rg3, address rg4, uint256 rg5) with:
             gas gas_remaining wei
            args cd[4], cd[36], Array(len=('cd', 68).length, data=mem[mem[64] + 196 len 32 * ('cd', 68).length]), address(this.address), cd[100]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _52 = mem[64]
        ...
        staticcall address(('cd', 68)[0]).balanceOf(address rg1) with:
                gas gas_remaining wei
               args this.address
        ...
        staticcall stor2.getAmountsOut(uint256 rg1, address[] rg2) with:
                gas gas_remaining wei
               args cd[4], Array(len=('cd', 68).length, data=mem[mem[64] + 100 len 32 * ('cd', 68).length])
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _89 = mem[64]
        mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
        ...
        if _91 - 1 >= _91:
            revert with 0, 50
        if mem[(32 * _91 - 1) + _89 + ceil32(return_data.size) + 32] < cd[36]:
        if idx == -1:
            revert with 0, 17
        idx = idx + 1
        continue 
