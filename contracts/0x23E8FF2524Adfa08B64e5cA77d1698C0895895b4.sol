...
function updateNFTFactory(address arg1) {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'wOwnable: caller is not the owne'
    factoryAddress = arg1
...
function updateNFTFactoryPrivate(address arg1) {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'wOwnable: caller is not the owne'
...
function registerRoyalty(address arg1, uint256 arg2, uint8 arg3) {
    require calldata.size - 4 >= 96
    if not sub_18944e55Address:
        revert with 0, 'Artion not set'
    if not factoryAddress:
        revert with 0, 'Factory not set'
    if sub_18944e55Address != arg1:
        require ext_code.size(factoryAddress)
        call factoryAddress.0xf6a3d24e with:
             gas gas_remaining wei
            args arg1
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        if not ext_call.return_data[0]:
            require ext_code.size(privateFactoryAddress)
            call privateFactoryAddress.0xf6a3d24e with:
                 gas gas_remaining wei
                args arg1
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
