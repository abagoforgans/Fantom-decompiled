...
function sub_5b55d141(?) {
    ...
    while idx < _77:
        ...
        t = t + 32
        continue 
    mem[_75 + 100] = this.address
    mem[_75 + 132] = block.timestamp + 15
...
function sub_06d0dd38(?) {
    ...
    while idx < _163:
        ...
        call address(_168).swapExactTokensForTokens(uint256 rg1, uint256 rg2, address[] rg3, address rg4, uint256 rg5) with:
             gas gas_remaining wei
            args _172, 0, 160, address(this.address), block.timestamp + 15, mem[mem[64] + 164 len (32 * _177) + 32]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _187 = mem[64]
        mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
...
function sub_47c71c21(?) {
    ...
    while idx < _150:
        ...
        continue 
    require ext_code.size(address(_146))
    staticcall address(_146).mem[mem[64] len 4] with:
            gas gas_remaining wei
           args mem[mem[64] + 4 len _149 + (32 * _150) + -mem[64] + 96]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    _217 = mem[64]
    mem[mem[64] len return_data.size] = ext_call.return_data[0 len return_data.size]
