...
mapping of uint256 sub_dd3b5f9a;
...
function sub_dd3b5f9a(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    return sub_dd3b5f9a[arg1]
}
...
function sub_a1287095(?) payable {
    ...
    while idx < ('cd', 36).length:
        if idx < cd[4]:
            revert with 'NH{q', 17
        if idx - cd[4] >= ('cd', 36).length:
            revert with 'NH{q', 50
        mem[0] = idx
        mem[32] = 11
        sub_dd3b5f9a[idx] = cd[((32 * idx - cd[4]) + cd[36] + 36)]
        if idx == -1:
            revert with 'NH{q', 17
        idx = idx + 1
        continue 
