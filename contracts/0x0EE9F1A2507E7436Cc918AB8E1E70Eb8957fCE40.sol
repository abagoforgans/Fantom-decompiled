...
function sub_fd598e56(?) payable {
    ...
    if not arg1:
        ...
        if not ext_code.hash(stor4):
            revert with 0, 'SafeERC20: call to non-contract'
        mem[ceil32(return_data.size) + 393 len 64] = unknown_0xa9059cbb(?????), msg.sender, Mask(224, 32, stor2 * arg1 / 10^18) >> 32
        call stor4 with:
           funct uint32(msg.sender)
             gas gas_remaining wei
            args Mask(224, 32, stor2 * arg1 / 10^18) << 224, mem[ceil32(return_data.size) + 457 len 4]
    if not return_data.size:
        require not ext_call.success
        revert with 0, 'SafeERC20: low-level call failed'
