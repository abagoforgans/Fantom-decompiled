...
function dev(address arg1) payable {
    ...
    if devaddr != msg.sender:
        revert with 0, 'dev: FORBIDDEN'
    if not arg1:
        revert with 0, 'dev: ZERO'
    devaddr = arg1
}
        ...
        if ext_call.return_data[0]:
            if poolInfo[arg1].field_256:
                ...
                if not block.number - poolInfo[arg1].field_512:
                    ...
                    call sub_eb010a95Address.mint(address rg1, uint256 rg2) with:
                         gas gas_remaining wei
                    ...
                    if not block.number - poolInfo[arg1].field_512:
                        ...
                        if not (block.number * sub_ca62b921) - (poolInfo[arg1].field_512 * sub_ca62b921):
                            ...
                            call sub_eb010a95Address.mint(address rg1, uint256 rg2) with:
                                 gas gas_remaining wei
                                args devaddr, (block.number * sub_ca62b921 * poolInfo[arg1].field_256) - (poolInfo[arg1].field_512 * sub_ca62b921 * poolInfo[arg1].field_256) / totalAllocPoint / 10
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            require ext_code.size(sub_eb010a95Address)
                            call sub_eb010a95Address.mint(address rg1, uint256 rg2) with:
            ...
            if not block.number - poolInfo[idx].field_512:
                ...
                if not (block.number * sub_ca62b921) - (poolInfo[idx].field_512 * sub_ca62b921):
                    ...
                    if not totalAllocPoint:
                        ...
                        call sub_eb010a95Address.mint(address rg1, uint256 rg2) with:
                             gas gas_remaining wei
                            args devaddr, (block.number * sub_ca62b921 * poolInfo[idx].field_256) - (poolInfo[idx].field_512 * sub_ca62b921 * poolInfo[idx].field_256) / totalAllocPoint / 10
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        mem[mem[64]] = 0x40c10f1900000000000000000000000000000000000000000000000000000000
                        mem[mem[64] + 4] = this.address
