...
uint8 sub_52b779ea; offset 16
address sub_e4d75cf8Address; offset 24
...
function toggleEffectiveCollateralRatio() payable {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    Mask(248, 0, stor15.field_8) = Mask(248, 0, not sub_2988b66d)
}

function sub_b2fb13c2(?) payable {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    Mask(240, 0, stor15.field_16) = Mask(240, 0, not sub_52b779ea)
    emit 0xbcf294fc: bool(0 or uint8(not sub_52b779ea))
}
...
function toggleCollateralRatio() payable {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    uint256(stor15.field_0) = not collateral_ratio_paused or Mask(248, 8, uint256(stor15.field_0))
    emit CollateralRatioToggled(bool(uint8(not collateral_ratio_paused)));
}

function sub_cc070ef0(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == address(arg1)
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if not address(arg1):
        revert with 0, '_oracle_twap address '
...
function sub_d33f1986(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == bool(arg1)
    if arg1:
        if not collateral_ratio_paused:
            ...
            if block.timestamp - sub_6ae74488 >= refresh_cooldown:
                if not sub_52b779ea:
                    ...
                    if not sub_e4d75cf8Address:
                        ...
                        call sub_e4d75cf8Address.update() with:
                             gas gas_remaining wei
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        require ext_code.size(sub_e4d75cf8Address)
                        ...
                        if ext_call.return_data[0] and 10^6 > -1 / ext_call.return_data[0]:
                            revert with 0, 17
                        if 10^6 * ext_call.return_data[0] / 10^18:
                            if price_target > !price_band:
                                revert with 0, 17
                            if 10^6 * ext_call.return_data[0] / 10^18 > price_target + price_band:
                                if target_collateral_ratio <= sub_ac4f843e:
                                    target_collateral_ratio = 0
                                else:
                                    if target_collateral_ratio < sub_ac4f843e:
