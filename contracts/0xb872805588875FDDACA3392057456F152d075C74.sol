...
function sub_6a2bd654(?) payable {
    ...
    if not ('cd', 4).length:
        idx = 0
        while uint256(sub_ae543cb2[cd[36]][address(msg.sender)].field_0) > idx:
            uint256(sub_ae543cb2[cd[36]][address(msg.sender)][idx].field_0) = 0
            idx = idx + 1
            continue 
    else:
        s = 0
        ...
        while (32 * ('cd', 4).length) + 128 > idx:
            uint256(sub_ae543cb2[cd[36]][address(msg.sender)][s].field_0) = mem[idx]
            s = s + 1
            idx = idx + 32
            continue 
