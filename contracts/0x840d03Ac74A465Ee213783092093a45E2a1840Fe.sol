...
function depositAll() payable {
    ...
    if not ext_call.return_data[0]:
        ...
        if not return_data.size:
            ...
            call address(ext_call.return_data[0]) with:
                 gas gas_remaining wei
                args ext_call.return_data[0] - (ext_call.return_data[0] * sub_54747a70 / 10000), 32, Mask(224, 32, 'SafeERC20: low-level call failed') >> 32, mem[ceil32(return_data.size) + 585 len 4]
            if not return_data.size:
                if not ext_call.success:
                    revert with unknown_0x23b872dd(?????), 
                                msg.sender,
