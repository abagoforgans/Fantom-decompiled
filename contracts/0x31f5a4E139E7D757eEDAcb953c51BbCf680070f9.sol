...
function sub_6ad1177d(?) payable {
    ...
    return address(sub_6ad1177d[arg1])
}
...
function sub_e0b0643f(?) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if sub_6ad1177d.length:
        revert with 0, 'only can be called once'
    sub_6ad1177d.length = ('cd', 4).length
    if not ('cd', 4).length:
        idx = 0
        while sub_6ad1177d.length > idx:
            uint256(sub_6ad1177d[idx]) = 0
            idx = idx + 1
            continue 
    else:
        s = 0
        ...
        while cd[4] + (32 * ('cd', 4).length) + 36 > idx:
            address(sub_6ad1177d[s]) = address(cd[idx])
            s = s + 1
            idx = idx + 32
            continue 
...
function draw() payable {
    ...
    while uint8(idx) < 5:
        ...
        sub_8c3b8688.length++
        mem[0] = 2
        sub_8c3b8688[sub_8c3b8688.length] = address(sub_6ad1177d[uint8(_16 % sub_6ad1177d.length)])
        if 1 > -uint8(idx) + 255:
            revert with 0, 17
        idx = uint8(idx) + 1
        continue 
