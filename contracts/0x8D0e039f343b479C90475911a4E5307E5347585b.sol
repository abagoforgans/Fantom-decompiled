...
function investedUnderlyingBalance() payable {
    ...
    staticcall stor2[('name', 'stor7265', 540221502481122645208940)].0x93f1a40b with:
            gas gas_remaining wei
...
function emergencyExit() payable {
    ...
    if msg.sender == ext_call.return_data[12 len 20]:
        require ext_code.size(stor2[('name', 'stor7265', 540221502481122645208940)])
        call stor2[('name', 'stor7265', 540221502481122645208940)].0x5312ea8e with:
             gas gas_remaining wei
            args stor1[('name', 'stor706F', 123623913048420)]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
