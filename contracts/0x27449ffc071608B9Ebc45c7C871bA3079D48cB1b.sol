...
function sub_3a091650(?) payable {
    ...
    if arg1.length > 0xdfb23b0979b4b02ebfbf567aef42d1c5127c92b10076d6af5d08a7fd98d5:
        revert with 0, 17
    if gas_remaining < 75000 * arg1.length:
        revert with 3891645165
    idx = 0
...
function adventure(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    if nextIndex >= stor6.length:
        nextIndex = 0
    if nextIndex > !arg1:
        revert with 0, 17
    if nextIndex + arg1 <= stor6.length:
        if arg1 > gas_remaining / 75000:
            revert with 3891645165
        idx = 0
        while idx < arg1:
            if nextIndex > !idx:
                revert with 0, 17
            if nextIndex + idx >= stor6.length:
                revert with 0, 50
            mem[0] = 6
            mem[96] = 0xb00b52f100000000000000000000000000000000000000000000000000000000
            mem[100] = stor[('name', 'stor6', 6) + stor8 + idx].field_0
            require ext_code.size(0xce761d788df608bd21bdd59d6f4b54b2e27f25bb)
            call 0xce761d788df608bd21bdd59d6f4b54b2e27f25bb.0xb00b52f1 with:
                 gas gas_remaining wei
                args stor[('name', 'stor6', 6) + stor8 + idx].field_0
            if idx == -1:
                revert with 0, 17
            idx = idx + 1
            continue 
        ...
        if stor6.length < nextIndex:
            revert with 0, 17
        if stor6.length - nextIndex > gas_remaining / 75000:
            revert with 3891645165
        idx = 0
        while idx < stor6.length - nextIndex:
            if nextIndex > !idx:
                revert with 0, 17
            if nextIndex + idx >= stor6.length:
                revert with 0, 50
            ...
            idx = idx + 1
            continue 
        if nextIndex > !(stor6.length - nextIndex):
            revert with 0, 17
        nextIndex = stor6.length
}
