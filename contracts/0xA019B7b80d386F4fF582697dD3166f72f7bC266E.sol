...
function sub_0fa4230d(?) {
    ...
    idx = 0
    while idx < cd[4]:
        if tokenByIndex.length < maxNFTs:
            _344 = mem[64]
            mem[64] = mem[64] + 32
            mem[_344] = 0
            if not msg.sender:
...
function sub_e5140e0a(?) payable {
    ...
    if cd[4] <= 0:
        revert with 0, 'Why are you minting less than zero nfts'
    if tokenByIndex.length > -cd[4] - 1:
        revert with 'NH{q', 17
    if tokenByIndex.length + cd[4] > maxNFTs:
        revert with 0, 'Only 1,000 NFTs are available'
    if cd[4] > sub_e735930b:
        revert with 0, 'Cannot mint this number of NFTs in one go !'
    if nFTPrice and cd[4] > -1 / nFTPrice:
        revert with 'NH{q', 17
    if nFTPrice * cd[4] > msg.value:
        revert with 0, 'Ethereum sent is not sufficient.'
    idx = 0
    while idx < cd[4]:
        if tokenByIndex.length < maxNFTs:
            _360 = mem[64]
