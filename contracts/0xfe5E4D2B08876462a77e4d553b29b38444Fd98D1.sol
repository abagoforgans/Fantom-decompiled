...
function getTicketsPurchased() {
    ...
    while 800 > idx + 32:
        ...
        s = s + 1
        continue 
    return ticketMapping.length, mem[480 len 320]
}
...
function sendReward() {
    ...
    call ticketMapping[hash % stor2] with:
       value 7 * ticketMax * stor1.length / 10 wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    emit 0x1ba25787: hash % ticketMax, 7 * ticketMax * stor1.length / 10, ticketMapping[hash % stor2]
