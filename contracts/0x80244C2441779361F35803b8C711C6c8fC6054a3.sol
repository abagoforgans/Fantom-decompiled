...
const pairCodeHash = sha3(code.data[1834 len 8962])

const INIT_CODE_PAIR_HASH = sha3(code.data[1834 len 8960], mem[18112 len 2])
...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'BoneFactory: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'BoneFactory: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'BoneFactory: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'BoneFactory: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[1834 len 8962]
