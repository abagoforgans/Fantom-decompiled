...
uint8 decimals;
address goldnuggetAddress; offset 8

function goldnugget() payable {
    return goldnuggetAddress
}
...
function leave(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    require ext_code.size(goldnuggetAddress)
    staticcall goldnuggetAddress.0x70a08231 with:
    ...
    if not arg1:
        ...
        require ext_code.size(goldnuggetAddress)
        call goldnuggetAddress.0xa9059cbb with:
             gas gas_remaining wei
...
function enter(uint256 arg1) payable {
    ...
    require ext_code.size(goldnuggetAddress)
    call goldnuggetAddress.0x23b872dd with:
         gas gas_remaining wei
