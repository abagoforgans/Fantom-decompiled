...
function upgrade() {
    ...
    if not ext_call.return_data[0]:
        revert with 0, 'Strategy Proxy: Upgrade not scheduled'
    if ext_code.size(ext_call.return_data[32]) <= 0:
    ...
    delegate this.address.mem[ceil32(return_data.size) + 132 len 4] with:
        ...
        args 
    if not delegate.return_code:
        revert with 0, 'Strategy Proxy: Issue when finalizing the upgrade'
}
