...
function mint(uint256 arg1) payable {
    ...
    if tokenByIndex.length + arg1 > maxSupply:
        revert with 0, 'Mint would exceed max supply'
    idx = 0
    while idx < arg1:
        if tokenByIndex.length > !idx:
            revert with 0, 17
        if 1 > !(tokenByIndex.length + idx):
            revert with 0, 17
        _126 = mem[64]
        mem[64] = mem[64] + 32
        mem[_126] = 0
        ...
        if ownerOf[stor8.length + idx + 1]:
            revert with 0, 'ERC721: token already minted'
        stor9[stor8.length + idx + 1] = tokenByIndex.length
        tokenByIndex.length++
        tokenByIndex[tokenByIndex.length] = tokenByIndex.length + idx + 1
        if msg.sender:
            if not msg.sender:
                if balanceOf[address(msg.sender)] > -2:
                    revert with 0, 17
                balanceOf[address(msg.sender)]++
                mem[0] = tokenByIndex.length + idx + 1
                mem[32] = 2
                ownerOf[stor8.length + idx + 1] = msg.sender
                emit Transfer(0, msg.sender, tokenByIndex.length + idx + 1);
                if ext_code.size(msg.sender):
                    mem[mem[64]] = 0x150b7a0200000000000000000000000000000000000000000000000000000000
                    mem[mem[64] + 4] = msg.sender
                    mem[mem[64] + 36] = 0
                    mem[mem[64] + 68] = tokenByIndex.length + idx + 1
                    mem[mem[64] + 100] = 128
                    mem[mem[64] + 132] = 0
                    s = 0
                    while s < 0:
                    ...
                    call msg.sender.onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
                         gas gas_remaining wei
                        args msg.sender, 0, tokenByIndex.length + idx + 1, 128, 0
                ...
                if not msg.sender:
                    revert with 0, 'ERC721: balance query for the zero address'
                tokenOfOwnerByIndex[address(msg.sender)][stor3[address(msg.sender)]] = tokenByIndex.length + idx + 1
                stor7[stor8.length + idx + 1] = balanceOf[address(msg.sender)]
                if balanceOf[address(msg.sender)] > -2:
                    revert with 0, 17
                balanceOf[address(msg.sender)]++
                mem[0] = tokenByIndex.length + idx + 1
            ...
            if tokenByIndex.length - 1 >= tokenByIndex.length:
                revert with 0, 50
            if stor9[stor8.length + idx + 1] >= tokenByIndex.length:
                revert with 0, 50
            tokenByIndex[stor9[tokenByIndex.length + idx + 1]] = tokenByIndex[tokenByIndex.length]
            stor9[stor8[stor8.length]] = stor9[stor8.length + idx + 1]
            stor9[stor8.length + idx + 1] = 0
            if not tokenByIndex.length:
                revert with 0, 49
            tokenByIndex[tokenByIndex.length] = 0
            tokenByIndex.length--
