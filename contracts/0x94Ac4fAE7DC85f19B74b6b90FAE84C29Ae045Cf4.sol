...
function addPool(uint256 arg1, address arg2, uint256 arg3, uint256 arg4) payable {
    ...
    while idx < poolInfo.length:
        ...
        _145 = mem[64]
        mem[64] = mem[64] + 192
        mem[_145] = poolInfo[idx].field_0
        mem[_145 + 32] = poolInfo[idx].field_256
        mem[_145 + 64] = poolInfo[idx].field_512
        mem[_145 + 96] = poolInfo[idx].field_768
        mem[_145 + 128] = poolInfo[idx].field_1024
        mem[_145 + 160] = poolInfo[idx].field_1280
        if block.timestamp > poolInfo[idx].field_512:
            mem[mem[64] + 4] = this.address
            require ext_code.size(poolInfo[idx].field_0)
            staticcall poolInfo[idx].field_0.0x70a08231 with:
            ...
            if not mem[_151]:
                ...
                mem[mem[64] + 32] = _166
                mem[mem[64] + 64] = poolInfo[idx].field_768
                emit 0x4bd31820: block.timestamp, _166, poolInfo[idx].field_768, idx
            else:
                if block.timestamp < poolInfo[idx].field_512:
                    revert with 0, 17
                if block.timestamp - poolInfo[idx].field_512 and rewardPerSecond > -1 / block.timestamp - poolInfo[idx].field_512:
                ...
                if not mem[_151]:
                    revert with 0, 18
                if poolInfo[idx].field_768 > !(10^12 * (block.timestamp * rewardPerSecond * poolInfo[idx].field_256) - (poolInfo[idx].field_512 * rewardPerSecond * poolInfo[idx].field_256) / totalAllocPoint / mem[_151]):
                    revert with 0, 17
                mem[_145 + 96] = poolInfo[idx].field_768 + (10^12 * (block.timestamp * rewardPerSecond * poolInfo[idx].field_256) - (poolInfo[idx].field_512 * rewardPerSecond * poolInfo[idx].field_256) / totalAllocPoint / mem[_151])
                mem[mem[64]] = 0xb4f56b2600000000000000000000000000000000000000000000000000000000
                mem[mem[64] + 4] = this.address
                mem[mem[64] + 36] = (block.timestamp * rewardPerSecond * poolInfo[idx].field_256) - (poolInfo[idx].field_512 * rewardPerSecond * poolInfo[idx].field_256) / totalAllocPoint
                require ext_code.size(rewardTokenAddress)
                ...
                poolInfo[idx].field_256 = poolInfo[idx].field_256
                poolInfo[idx].field_512 = block.timestamp
                poolInfo[idx].field_768 = mem[_145 + 96]
                poolInfo[idx].field_1024 = poolInfo[idx].field_1024
                poolInfo[idx].field_1280 = poolInfo[idx].field_1280
                _195 = mem[_145 + 96]
                mem[mem[64]] = block.timestamp
                mem[mem[64] + 32] = _166
                mem[mem[64] + 64] = _195
                emit 0x4bd31820: block.timestamp, _166, _195, idx
        if idx == -1:
            revert with 0, 17
        idx = idx + 1
        continue 
