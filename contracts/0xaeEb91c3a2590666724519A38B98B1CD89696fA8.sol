...
function sub_48334429(?) payable {
    ...
    if not -_42 + (arg2.length * _42):
        return 0
    if (-10000 * _42) + (10000 * arg2.length * _42) + (_42 * slippage) - (arg2.length * _42 * slippage) / -_42 + (arg2.length * _42) != -slippage + 10000:
        revert with 0, 32, 33, 0x2e536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[(32 * arg2.length) + 293 len 31]
    return ((-10000 * _42) + (10000 * arg2.length * _42) + (_42 * slippage) - (arg2.length * _42 * slippage) / 10000)
}
...
function health(uint256 arg1) payable {
    ...
    if 0 == totalShare:
        require ext_code.size(address(lpTokenAddress))
        staticcall address(lpTokenAddress).0x18160ddd with:
                gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        require ext_code.size(address(lpTokenAddress))
        staticcall address(lpTokenAddress).getReserves() with:
                gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
