...
function sub_b54221bf(?) payable {
    ...
    while idx < 2:
        ...
        t = t + 32
        continue 
    mem[ceil32(return_data.size) + 292] = address(owner)
    ...
    call stor3.swapExactTokensForTokens(uint256 rg1, uint256 rg2, address[] rg3, address rg4, uint256 rg5) with:
         gas gas_remaining wei
        args arg1, arg2, Array(len=2, data=mem[ceil32(return_data.size) + 388 len 64]), address(owner), block.timestamp + 3600
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
