...
function sub_2a6294bc(?) payable {
    ...
    if stor6 > -arg1 - 1:
        revert with 'NH{q', 17
    if stor6:
        if stor6 + arg1 <= userCount:
            idx = stor6
            while idx <= stor6 + arg1:
                mem[0] = users[idx]
                mem[32] = 8
                if stor4 and impact[stor9[idx]] > -1 / stor4:
                    revert with 'NH{q', 17
                if not totalLocked:
                    revert with 'NH{q', 18
                mem[96] = 0xca0b187800000000000000000000000000000000000000000000000000000000
                mem[100] = users[idx]
                mem[132] = stor4 * impact[stor9[idx]] / totalLocked
                require ext_code.size(balanceKeeperAddress)
                call balanceKeeperAddress.0xca0b1878 with:
                     gas gas_remaining wei
                    args users[idx], stor4 * impact[stor9[idx]] / totalLocked
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                if idx == -1:
                    revert with 'NH{q', 17
        ...
        if stor6 + arg1 <= userCount:
            ...
            while idx <= stor6 + arg1:
                ...
                mem[ceil32(return_data.size) + 132] = stor4 * impact[stor9[idx]] / totalLocked
                require ext_code.size(balanceKeeperAddress)
                call balanceKeeperAddress.0xca0b1878 with:
                     gas gas_remaining wei
                    args users[idx], stor4 * impact[stor9[idx]] / totalLocked
