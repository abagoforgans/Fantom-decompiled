        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if (periodFinish * rewardRate) - (block.timestamp * rewardRate) / periodFinish - block.timestamp != rewardRate:
                    revert with 0, 32, 33, 0x73536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
                if (periodFinish * rewardRate) - (block.timestamp * rewardRate) + ext_call.return_data[0] < ext_call.return_data[0]:
                    revert with 0, 'SafeMath: addition overflow'
                if not duration:
                    revert with 0, 'SafeMath: division by zero'
                rewardRate = (periodFinish * rewardRate) - (block.timestamp * rewardRate) + ext_call.return_data[0] / duration
    else:
        if block.timestamp < periodFinish:
            if lastUpdateTime > block.timestamp:
                revert with 0, 'SafeMath: subtraction overflow'
...
function withdraw(uint256 arg1) payable {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            ...
            if 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 == ext_code.hash(stakedTokenAddress):
                revert with 0, 'SafeERC20: call to non-contract'
