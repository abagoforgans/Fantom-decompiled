    ...
    if not return_data.size:
        ...
        if not totalSupply:
            ...
            if not 0 / 2 * ext_call.return_data[0]:
                ...
                if not msg.sender:
                    revert with 0, 'ERC20: mint to the zero address'
                if totalSupply + (0 / 2 * ext_call.return_data[0] * depoPercent / 100) < totalSupply:
                    revert with 0, 'SafeMath: addition overflow'
                totalSupply += 0 / 2 * ext_call.return_data[0] * depoPercent / 100
                if balanceOf[address(msg.sender)] + (0 / 2 * ext_call.return_data[0] * depoPercent / 100) < balanceOf[address(msg.sender)]:
                    revert with 0, 'SafeMath: addition overflow'
                balanceOf[address(msg.sender)] += 0 / 2 * ext_call.return_data[0] * depoPercent / 100
                emit Transfer((0 / 2 * ext_call.return_data[0] * depoPercent / 100), 0, msg.sender);
            require ext_code.size(tokenAddress)
            staticcall tokenAddress.0x70a08231 with:
                    gas gas_remaining wei
                   args this.address
...
function depositAll() payable {
    ...
    if not return_data.size:
        ...
        if not totalSupply:
            ...
            if ext_code.size(tokenAddress) <= 0:
                revert with 0, 'Address: call to non-contract'
            mem[648 len 64] = unknown_0xa9059cbb(?????), Mask(224, 0, stor12), uint32(stor12), ext_call.return_data[0 len 28]
            mem[712 len 0] = 0
            call tokenAddress with:
               funct uint32(stor12)
                 gas gas_remaining wei
        ...
        if return_data.size <= 0:
            ...
            if not totalSupply:
                ...
                if not 0 / 2 * ext_call.return_data[0]:
                    ...
                    if ext_code.size(tokenAddress) <= 0:
                        revert with 0, 'Address: call to non-contract'
                    mem[ceil32(return_data.size) + 649 len 64] = unknown_0xa9059cbb(?????), Mask(224, 0, stor12), uint32(stor12), ext_call.return_data[0 len 28]
