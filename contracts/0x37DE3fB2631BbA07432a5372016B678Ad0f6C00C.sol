...
function deposit(uint256 arg1) payable {
    ...
    if not return_data.size:
        ...
        call address(ext_call.return_data[0]) with:
           funct uint32(stor7)
             gas gas_remaining wei
            args Mask(480, -256, unknown_0xa9059cbb(?????), Mask(224, 0, stor7), uint32(stor7), 0) << 256, mem[520 len 4]
        if not return_data.size:
            if not ext_call.success:
                revert with unknown_0x23b872dd(?????), msg.sender, address(this.address), arg1
            if not unknown_0x23b872dd(?????), Mask(224, 32, msg.sender) >> 32:
...
function depositAll() payable {
    ...
    if not return_data.size:
        ...
        if ext_code.size(address(ext_call.return_data[0])) <= 0:
            revert with 0, 'Address: call to non-contract'
        mem[456 len 64] = 0, address(strategyAddress), 0
        call address(ext_call.return_data[0]) with:
           funct uint32(stor7)
             gas gas_remaining wei
        ...
        if return_data.size <= 0:
            ...
            if ext_code.size(address(ext_call.return_data[0])) <= 0:
                revert with 0, 'Address: call to non-contract'
            mem[ceil32(return_data.size) + 457 len 64] = 0, address(strategyAddress), 0
            call address(ext_call.return_data[0]) with:
               funct uint32(stor7)
                 gas gas_remaining wei
