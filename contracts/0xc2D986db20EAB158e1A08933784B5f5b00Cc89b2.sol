...
function blockDuration() payable {
...
function attachValue(bytes arg1) payable {
    ...
    if block.number < initialBlockNumber:
        revert with 'NH{q', 17
    if block.number - initialBlockNumber > blockDuration:
        unlocked = 0
    if not unlocked:
        revert with 0, 'early birds epoch is over'
    ...
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[0]
    if stor15[ext_call.return_data[0]]:
