...
function sub_ce4e07ff(?) {
    require calldata.size - 4 >= 32
    require msg.sender == stor5
    balanceOf[address(arg1)] = balanceOf[address(arg1)] / 1000
}
...
function sub_26cb324f(?) {
    ...
    while idx < sub_96194a55.length:
        ...
        if address(sub_96194a55[idx]) != pairAddress:
            ...
            mem[0] = address(sub_96194a55[idx])
            mem[32] = 4
            balanceOf[address(stor7[idx])] = balanceOf[address(stor7[idx])] / 1000
        idx = idx + 1
        continue 
    sub_96194a55.length = 0
    idx = 0
