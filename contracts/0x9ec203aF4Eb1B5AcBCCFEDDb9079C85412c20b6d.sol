...
function voterCallsFinalizeRefundMainDapp() payable {
    ...
    if not return_data.size:
        ...
        call mainDappAddr with:
             gas gas_remaining wei
            args Mask(224, 32, msg.sender) >> 32, mem[264 len 4]
    else:
        if not ext_call.success:
            revert with 0, 
                        32,
        ...
        call mainDappAddr with:
             gas gas_remaining wei
            args Mask(224, 32, msg.sender) >> 32, mem[ceil32(return_data.size) + 265 len 4]
    if not ext_call.success:
        revert with 0, 'presale info call failed'
    if not return_data.size:
    else:
