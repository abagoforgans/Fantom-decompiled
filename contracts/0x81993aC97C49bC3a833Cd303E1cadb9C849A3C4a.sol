...
function sub_f9c019d5(?) payable {
    ...
    staticcall address(ext_call.return_data[0]).getShareValue() with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    ...
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[31 len 1]
    return ext_call.return_data[0], 10^uint8(ext_call.return_data[0]), ext_call.return_data[0] / 10^uint8(ext_call.return_data[0])
}
