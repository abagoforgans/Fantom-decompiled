...
function sub_cca23c2b(?) payable {
    require ext_code.size(artGalleryAddress)
    call artGalleryAddress.0x65b1c3c with:
         gas gas_remaining wei
        args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    if ext_call.return_data[96]:
        require ext_code.size(artGalleryAddress)
        call artGalleryAddress.unlock() with:
             gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
}
