...
function mint(address arg1, string arg2) payable {
    ...
    if stor0 != msg.sender:
        revert with 0, 'ERC721: caller is not the owner'
    if stor1 == -1:
        revert with 'NH{q', 17
    stor1++
    if stor5[stor1].field_0:
        if stor5[stor1].field_0 == stor5[stor1].field_1 < 32:
            revert with 'NH{q', 34
        if arg2.length:
            stor5[stor1][].field_0 = Array(len=arg2.length, data=arg2[all])
        else:
            stor5[stor1].field_0 = 0
            idx = 0
            while stor5[stor1].field_1 + 31 / 32 > idx:
