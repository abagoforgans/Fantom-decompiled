...
function sub_3cd03be1(?) payable {
    return address(sub_3cd03be1Address)
}
...
function balanceOfPool() payable {
    require ext_code.size(address(sub_3cd03be1Address))
    staticcall address(sub_3cd03be1Address).userInfo(address arg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function deposit() payable {
    ...
    require return_data.size >= 32
    require ext_code.size(address(sub_3cd03be1Address))
    call address(sub_3cd03be1Address).deposit(uint256 arg1) with:
         gas gas_remaining wei
...
function retireStrat() payable {
    if address(vaultAddress) != msg.sender:
        revert with 0, '!vault'
    require ext_code.size(address(sub_3cd03be1Address))
    call address(sub_3cd03be1Address).emergencyWithdraw() with:
         gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(wantAddress)
...
function unpause() payable {
    ...
    staticcall wantAddress.0xdd62ed3e with:
            gas gas_remaining wei
           args address(this.address), address(sub_3cd03be1Address)
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    ...
    call wantAddress with:
       funct uint32(stor18)
         gas gas_remaining wei
        args Mask(480, -256, approve(address arg1, uint256 arg2), Mask(224, 0, stor18), uint32(stor18), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffff) << 256, mem[324 len 4]
    if not return_data.size:
        if not ext_call.success:
...
function withdraw(uint256 arg1) payable {
    ...
    require ext_code.size(address(sub_3cd03be1Address))
    call address(sub_3cd03be1Address).0x2e1a7d4d with:
         gas gas_remaining wei
