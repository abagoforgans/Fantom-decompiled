...
function claim(uint256 arg1) payable {
    ...
    if uint8(stor4.field_168):
        ...
        if block.timestamp - lastLimitTimestamp[msg.sender] > 24 * 3600:
            ...
            limitMax[msg.sender].field_0 = ext_call.return_data[0] / 2
            limitMax[msg.sender].field_255 = 0
        if arg1 > limitMax[msg.sender].field_0:
            revert with 0, 'C3'
        if limitMax[msg.sender].field_0 < arg1:
            revert with 0, 17
        limitMax[msg.sender].field_0 -= arg1
    require ext_code.size(balanceKeeperAddress)
